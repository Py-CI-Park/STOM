...
from utility.static import now, strf_time, timedelta_sec, int_hms_utc, GetBinanceShortPgSgSp, GetBinanceLongPgSgSp, threading_timer
...
class BinanceTrader:
    ...
    def UpdateJango(self, data):
        ...
        try:
            if code in self.df_jg.index and c != self.df_jg['현재가'][code]:
                ...
                if ps == 'LONG':
                    pg, sg, sp = GetBinanceLongPgSgSp(jg, jc * c, '시장가' in self.dict_set['코인매수주문구분'],
                                                      '시장가' in self.dict_set['코인매도주문구분'])
                else:
                    pg, sg, sp = GetBinanceShortPgSgSp(jg, jc * c, '시장가' in self.dict_set['코인매수주문구분'],
                                                       '시장가' in self.dict_set['코인매도주문구분'])
    ...
    def CheckOrder(self, data):
        ...
        if manual:
            ...
            if self.dict_set['코인매수금지거래횟수'] and self.dict_set['코인매수금지거래횟수값'] <= len(self.df_td[self.df_td['종목명'] == code].drop_duplicates('체결시간')):
                cancel = True
            elif self.dict_set['코인매수금지손절횟수'] and self.dict_set['코인매수금지손절횟수값'] <= len(self.df_td[(self.df_td['종목명'] == code) & (self.df_td['수익률'] < 0)].drop_duplicates('체결시간')):
                cancel = True
            elif NIJ and inthmsutc < self.dict_set['코인전략종료시간'] and len(self.df_jg) >= self.dict_set['코인최대매수종목수']:
                cancel = True
            elif self.dict_set['코인매수금지간격'] and curr_time < self.dict_time[code][2]:
                cancel = True
            elif self.dict_set['코인매수금지손절간격'] and curr_time < self.dict_time[code][3]:
                cancel = True
            elif og == 'BUY_LONG' and not NIJ and self.df_jg['분할매수횟수'][code] >= self.dict_set['코인매수분할횟수']:
                cancel = True
            elif og == 'SELL_SHORT' and not NIJ and self.df_jg['분할매수횟수'][code] >= self.dict_set['코인매수분할횟수']:
                cancel = True
            elif self.dict_intg['예수금'] < oc * op:
        ...
        if cancel:
            ...
            if og == 'BUY_LONG':
                if self.dict_set['코인매도취소매수시그널'] and INSL:
                    self.CancelOrder(code, og)
            elif og == 'SELL_SHORT':
                if self.dict_set['코인매도취소매수시그널'] and INBS:
                    self.CancelOrder(code, og)
            elif og == 'SELL_LONG':
                if self.dict_set['코인매수취소매도시그널'] and INBL:
                    self.CancelOrder(code, og)
            elif og == 'BUY_SHORT':
                if self.dict_set['코인매수취소매도시그널'] and INSS:
                    self.CancelOrder(code, og)
        ...
        if oc > 0:
            if self.dict_set['코인모의투자'] or og == '시드부족':
                ...
                if og == '시드부족':
                    ...
                    if og == 'BUY_LONG':
                        self.dict_buy_long[code] = [0, timedelta_sec(self.dict_set['코인매수취소시간초']), fixc, op, self.dict_lvrg[code]]
                    elif og == 'SELL_SHORT':
    ...
    def OrderTimeControl(self, code=None):
        if code is None:
            if len(self.dict_buy_long) > 0:
                buy_long_cancel_list = []
                buy_long_modify_list = []
                for code, order_info in self.dict_buy_long.items():
                    if self.dict_set['코인매수취소시간'] and now() > order_info[1]:
                        buy_long_cancel_list.append(code)
                    elif order_info[2] < self.dict_set['코인매수정정횟수'] and code in self.dict_curc.keys() and \
                            self.dict_curc[code] >= order_info[3] + self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가차이']:
                        buy_long_modify_list.append(code)
                if len(buy_long_cancel_list) > 0:
                    for code in buy_long_cancel_list:
                        self.CancelOrder(code, 'BUY_LONG')
                if len(buy_long_modify_list) > 0:
                    for code in buy_long_modify_list:
                        self.ModifyOrder(code, 'BUY_LONG')

            if len(self.dict_sell_short) > 0:
                sell_short_cancel_list = []
                sell_short_modify_list = []
                for code, order_info in self.dict_sell_short.items():
                    if self.dict_set['코인매수취소시간'] and now() > order_info[1]:
                        sell_short_cancel_list.append(code)
                    elif order_info[2] < self.dict_set['코인매수정정횟수'] and code in self.dict_curc.keys() and \
                            self.dict_curc[code] <= order_info[3] - self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가차이']:
                        sell_short_modify_list.append(code)
                if len(sell_short_cancel_list) > 0:
                    for code in sell_short_cancel_list:
                        self.CancelOrder(code, 'SELL_SHORT')
                if len(sell_short_modify_list) > 0:
                    for code in sell_short_modify_list:
                        self.ModifyOrder(code, 'SELL_SHORT')

            if len(self.dict_sell_long) > 0:
                sell_long_cancel_list = []
                sell_long_modify_list = []
                for code, order_info in self.dict_sell_long.items():
                    if self.dict_set['코인매도취소시간'] and now() > order_info[1]:
                        sell_long_cancel_list.append(code)
                    elif order_info[2] < self.dict_set['코인매도정정횟수'] and code in self.dict_curc.keys() and \
                            self.dict_curc[code] <= order_info[3] - self.dict_info[code]['호가단위'] * self.dict_set['코인매도정정호가차이']:
                        sell_long_modify_list.append(code)
                if len(sell_long_cancel_list) > 0:
                    for code in sell_long_cancel_list:
                        self.CancelOrder(code, 'SELL_LONG')
                if len(sell_long_modify_list) > 0:
                    for code in sell_long_modify_list:
                        self.ModifyOrder(code, 'SELL_LONG')

            if len(self.dict_buy_short) > 0:
                buy_short_cancel_list = []
                buy_short_modify_list = []
                for code, order_info in self.dict_buy_short.items():
                    if self.dict_set['코인매수취소시간'] and now() > order_info[1]:
                        buy_short_cancel_list.append(code)
                    elif order_info[2] < self.dict_set['코인매수정정횟수'] and code in self.dict_curc.keys() and \
                            self.dict_curc[code] >= order_info[3] + self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가차이']:
                        buy_short_modify_list.append(code)
                if len(buy_short_cancel_list) > 0:
                    for code in buy_short_cancel_list:
                        self.CancelOrder(code, 'BUY_SHORT')
                if len(buy_short_modify_list) > 0:
                    for code in buy_short_modify_list:
                        self.ModifyOrder(code, 'BUY_SHORT')
        else:
            if code in self.dict_buy_long.keys():
                buy_long_cancel = False
                buy_long_modify = False
                order_info      = self.dict_buy_long[code]
                if self.dict_set['코인매수취소시간'] and now() > order_info[1]:
                    buy_long_cancel = False
                elif order_info[2] < self.dict_set['코인매수정정횟수'] and code in self.dict_curc.keys() and \
                        self.dict_curc[code] >= order_info[3] + self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가차이']:
                    buy_long_modify = False
                if buy_long_cancel:   self.CancelOrder(code, 'BUY_LONG')
                elif buy_long_modify: self.ModifyOrder(code, 'BUY_LONG')

            if code in self.dict_sell_short.keys():
                sell_short_cancel = False
                sell_short_modify = False
                order_info        = self.dict_sell_short[code]
                if self.dict_set['코인매수취소시간'] and now() > order_info[1]:
                    sell_short_cancel = False
                elif order_info[2] < self.dict_set['코인매수정정횟수'] and code in self.dict_curc.keys() and \
                        self.dict_curc[code] <= order_info[3] - self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가차이']:
                    sell_short_modify = False
                if sell_short_cancel:   self.CancelOrder(code, 'SELL_SHORT')
                elif sell_short_modify: self.ModifyOrder(code, 'SELL_SHORT')

            if code in self.dict_sell_long.keys():
                sell_long_cancel = False
                sell_long_modify = False
                order_info       = self.dict_sell_long[code]
                if self.dict_set['코인매도취소시간'] and now() > order_info[1]:
                    sell_long_cancel = False
                elif order_info[2] < self.dict_set['코인매도정정횟수'] and code in self.dict_curc.keys() and \
                        self.dict_curc[code] <= order_info[3] - self.dict_info[code]['호가단위'] * self.dict_set['코인매도정정호가차이']:
                    sell_long_modify = False
                if sell_long_cancel:   self.CancelOrder(code, 'SELL_LONG')
                elif sell_long_modify: self.ModifyOrder(code, 'SELL_LONG')

            if code in self.dict_buy_short.keys():
                buy_short_cancel = False
                buy_short_modify = False
                order_info       = self.dict_buy_short[code]
                if self.dict_set['코인매수취소시간'] and now() > order_info[1]:
                    buy_short_cancel = True
                elif order_info[2] < self.dict_set['코인매수정정횟수'] and code in self.dict_curc.keys() and \
                        self.dict_curc[code] >= order_info[3] + self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가차이']:
                    buy_short_modify = True
                if buy_short_cancel:   self.CancelOrder(code, 'BUY_SHORT')
                elif buy_short_modify: self.ModifyOrder(code, 'BUY_SHORT')
    ...
    def ModifyOrder(self, code, gubun):
        ...
        if len(df) > 0:
            ...
            if mc > 0:
                ...
                if gubun == 'BUY_LONG':
                    jjop = self.dict_curc[code] - self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가']
                    fixc = self.dict_buy_long[code][2] + 1
                elif gubun == 'SELL_SHORT':
                    jjop = self.dict_curc[code] + self.dict_info[code]['호가단위'] * self.dict_set['코인매수정정호가']
                    fixc = self.dict_sell_short[code][2] + 1
                elif gubun == 'SELL_LONG':
                    jjop = self.dict_curc[code] + self.dict_info[code]['호가단위'] * self.dict_set['코인매도정정호가']
                    fixc = self.dict_sell_long[code][2] + 1
                else:
                    jjop = self.dict_curc[code] - self.dict_info[code]['호가단위'] * self.dict_set['코인매도정정호가']
                    fixc = self.dict_buy_short[code][2] + 1

                curr_time = now()
                self.CreateOrder(f'{gubun}_CANCEL', code, op, mc, on, curr_time, False, 0, None)
                self.CreateOrder(gubun, code, jjop, mc, '', curr_time, False, fixc, None)
    ...
    def UpdateTotaljango(self):
        ...
        if self.dict_set['코인투자금고정']:
            tujagm = int(self.dict_set['코인투자금'])
        else:
            tujagm = int(self.dict_intg['추정예탁자산'] * 0.98 / self.dict_set['코인최대매수종목수'])
