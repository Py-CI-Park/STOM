...
class UpbitTrader:
    ...
    def CheckOrder(self, data):
        ...
        if cancel:
            ...
            if og == '매수':
                if self.dict_set['코인매도취소매수시그널'] and INS:
                    self.CancelOrder(code, og)
            elif og == '매도':
                if self.dict_set['코인매수취소매도시그널'] and INB:
                    self.CancelOrder(code, og)
    ...
    def CreateOrder(self, og, code, op, oc, on, signal_time, manual, fixc, ordertype):
        if og == '매수' and fixc == 0:
            if ordertype is None and '지정가' in self.dict_set['코인매수주문구분']:
                op = round(op + GetUpbitHogaunit(op) * self.dict_set['코인매수지정가호가번호'], 8)
        elif og == '매도' and fixc == 0:
            if ordertype is None and '지정가' in self.dict_set['코인매도주문구분']:
                op = round(op + GetUpbitHogaunit(op) * self.dict_set['코인매도지정가호가번호'], 8)
        ...
        if code is None:
            if len(self.dict_buy) > 0:
                ...
                for code, order_info in self.dict_buy.items():
                    ...
                    # [ret['uuid'], timedelta_sec(self.dict_set['코인매수취소시간초']), fixc, op, GetUpbitHogaunit(op)]
    ...
    def ModifyOrder(self, code, gubun):
        ...
        if len(df) > 0:
            ...
            if mc > 0:
                on, op = df['주문번호'].iloc[-1], df['주문가격'].iloc[-1]
                if gubun == '매수':
                    jjop = self.dict_curc[code] - self.dict_buy[code][4] * self.dict_set['코인매수정정호가']
                    fixc = self.dict_buy[code][2] + 1
                else:
                    jjop = self.dict_curc[code] + self.dict_sell[code][4] * self.dict_set['코인매도정정호가']
                    fixc = self.dict_sell[code][2] + 1

                curr_time = now()
                self.CreateOrder(f'{gubun}취소', code, op, mc, on, curr_time, False, 0, None)
    ...
    def UpdateTotaljango(self):
        ...
        if self.dict_set['코인투자금고정']:
            tujagm = int(self.dict_set['코인투자금'] * 1_000_000)
        else:
