...
class TraderKiwoom:
    def __init__(self, qlist):
        ...
        # noinspection PyUnresolvedReferences
        self.updater.signal2.connect(self.UpdateDictset)
        # noinspection PyUnresolvedReferences
        self.updater.signal3.connect(self.UpdateStr)
        self.updater.start()
    ...
    def LoadDatabase(self):
        ...
        if len(self.df_cj) > 0: self.kwmservQ.put(['window', [ui_num['S체결목록'], self.df_cj[::-1]]])
        if len(self.df_td) > 0: self.kwmservQ.put(['window', [ui_num['S거래목록'], self.df_td[::-1]]])
        if self.dict_set['주식모의투자']:
            self.df_jg = pd.read_sql('SELECT * FROM s_jangolist', con).set_index('index')
            if len(self.df_jg) > 0: self.sreceivQ.put(['잔고목록', list(self.df_jg.index)])
        con.close()

        self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 실행 알림 - 데이터베이스 정보 불러오기 완료']])

    def KiwoomLogin(self):
        self.kw.CommConnect()
        self.dict_strg['계좌번호'] = self.kw.GetAccountNumber()
        self.list_kosd = self.kw.GetCodeListByMarket('10')
        list_code = self.kw.GetCodeListByMarket('0') + self.list_kosd
        ...
        self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 실행 알림 - OpenAPI 로그인 완료']])
    ...
    def UpdateList(self, data):
        if len(data) == 7:
            self.CheckOrder(data[0], data[1], data[2], data[3], data[4], data[5], data[6])
        elif len(data) == 8:
            self.CheckOrder(data[0], data[1], data[2], data[3], data[4], data[5], data[6], ordertype=data[7])
        elif len(data) == 2:
            if data[0] == '관심진입':
    ...
    def UpdateStr(self, data):
        if data == 'S체결목록':
            self.kwmservQ.put(['tele', self.df_cj.copy()]) if len(self.df_cj) > 0 else self.kwmservQ.put(['tele', '현재는 주식체결목록이 없습니다.'])
        elif data == 'S거래목록':
            self.kwmservQ.put(['tele', self.df_td.copy()]) if len(self.df_td) > 0 else self.kwmservQ.put(['tele', '현재는 주식거래목록이 없습니다.'])
        elif data == 'S잔고평가':
            self.kwmservQ.put(['tele', self.df_jg.copy()]) if len(self.df_jg) > 0 else self.kwmservQ.put(['tele', '현재는 주식잔고목록이 없습니다.'])
        elif data == 'S잔고청산':
    ...
    def CheckOrder(self, gubun, code, name, op, oc, signal_time, manual, ordertype=None):
        ...
        if manual:
            ...
            inthms = int_hms()
            if self.dict_set['주식매수금지거래횟수'] and self.dict_set['주식매수금지거래횟수값'] <= len(self.df_td[self.df_td['종목명'] == name].drop_duplicates('체결시간')):
                cancel = True
            elif self.dict_set['주식매수금지손절횟수'] and self.dict_set['주식매수금지손절횟수값'] <= len(self.df_td[(self.df_td['종목명'] == name) & (self.df_td['수익률'] < 0)].drop_duplicates('체결시간')):
                cancel = True
            elif NIJ and inthms < self.dict_set['주식장초전략종료시간'] and len(self.df_jg) >= self.dict_set['주식장초최대매수종목수']:
                cancel = True
            elif NIJ and inthms > self.dict_set['주식장초전략종료시간'] and len(self.df_jg) >= self.dict_set['주식장중최대매수종목수']:
                cancel = True
        ...
        if cancel:
            ...
            if gubun == '매수':
                if self.dict_set['주식매도취소매수시그널'] and INS:
                    self.CancelOrder(code, gubun)
            elif gubun == '매도':
                if self.dict_set['주식매수취소매도시그널'] and INB:
                    self.CancelOrder(code, gubun)
    ...
    def CreateOrder(self, gubun, code, name, op, oc, on, signal_time, manual, ordertype):
        ...
        if manual:
            ...
            if self.dict_set['주식매수주문구분'] in ['지정가', '지정가IOC', '지정가FOK']:
                op += GetHogaunit(code in self.list_kosd, op, self.int_hgtime) * self.dict_set['주식매수지정가호가번호']
            ...
            if self.dict_set['주식매도주문구분'] in ['지정가', '지정가IOC', '지정가FOK']:
                op += GetHogaunit(code in self.list_kosd, op, self.int_hgtime) * self.dict_set['주식매도지정가호가번호']
    ...
    def SendOrder(self, order, name, signal_time):
        ...
        if ret == 0:
            self.kwmservQ.put(['window', [ui_num['S로그텍스트'], f'주문 관리 시스템 알림 - [전송] {name} | {order[6]} | {order[5]} | {order[0]}']])
            ...
            self.kwmservQ.put(['window', [ui_num['S로그텍스트'], f'시스템 명령 오류 알림 - [주문 실패] {name} | {order[6]} | {order[5]} | {order[0]}']])
    ...
    def Scheduler(self):
        ...
        if self.dict_intg['장운영상태'] in [2, 3]:
            if (not self.dict_bool['장초전략잔고청산'] and inthms < self.dict_set['주식장초전략종료시간']) or \
                    (not self.dict_set['주식장초프로세스종료'] and not self.dict_bool['장중전략잔고청산'] and inthms < self.dict_set['주식장중전략종료시간']):
    ...
    def GetAccountjanGo(self):
        ...
        while True:
            ...
            if df['D+2추정예수금'][0] != '':
                ...
                self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 오류 알림 - 오류가 발생하여 계좌평가현황을 재조회합니다.']])
                qtest_qwait(3.35)
        ...
        while True:
            ...
            if df['추정예탁자산'][0] != '':
                ...
                self.kwmservQ.put(['window', [ui_num['S잔고평가'], self.df_tj]])
                break
            else:
                self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 오류 알림 - 오류가 발생하여 계좌평가결과를 재조회합니다.']])
                qtest_qwait(3.35)

        if len(self.df_td) > 0:
            self.UpdateTotaltradelist(first=True)
        self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 실행 알림 - 계좌 조회 완료']])

    def OperationRealreg(self):
        self.dict_bool['트레이더시작'] = True
        self.kw.SetRealReg([sn_oper, ' ', '215;20;214', 0])
        self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 실행 알림 - 장운영시간 등록 완료']])
        self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 실행 알림 - 트레이더 시작']])
    ...
    def JangoCheongsan(self, gubun):
        ...
        if (gubun == '수동' or self.dict_set[f'주식{gubun}잔고청산']) and len(self.df_jg) > 0:
            ...
            if self.dict_set['주식알림소리']:
                self.kwmservQ.put(['sound', f'주식 {gubun}전략 잔고청산 주문을 전송하였습니다.'])
            self.kwmservQ.put(['window', [ui_num['S로그텍스트'], f'시스템 명령 실행 알림 - {gubun}전략 잔고청산 주문 완료']])

    def OrderTimeControl(self):
        buy_cancel_list = []
        buy_modify_list = []
        for code, order_info in self.dict_buy.items():
            if self.dict_set['주식매수취소시간'] and now() > order_info[0]:
                buy_cancel_list.append(code)
            if order_info[1] < self.dict_set['주식매수정정횟수'] and code in self.dict_curc.keys() and \
                ...
                buy_modify_list.append(code)

        if len(buy_cancel_list) > 0:
            for code in buy_cancel_list:
                self.CancelOrder(code, '매수')
        if len(buy_modify_list) > 0:
            for code in buy_modify_list:
                self.ModifyOrder(code, '매수')

        sell_cancel_list = []
        sell_modify_list = []
        for code, order_info in self.dict_sell.items():
            if self.dict_set['주식매도취소시간'] and now() > order_info[0]:
                sell_cancel_list.append(code)
            if order_info[1] < self.dict_set['주식매도정정횟수'] and code in self.dict_curc.keys() and \
                ...
                sell_modify_list.append(code)

        if len(sell_cancel_list) > 0:
            for code in sell_cancel_list:
                self.CancelOrder(code, '매도')
        if len(sell_modify_list) > 0:
            for code in sell_modify_list:
                self.ModifyOrder(code, '매도')
    ...
    def ModifyOrder(self, code, gubun):
        name = self.dict_name[code][0]
        df = self.GetMichegeolDF(name, gubun)
        if len(df) > 0:
            mc = df['미체결수량'].iloc[-1]
            if mc > 0:
                if gubun == '매수':
                    op = self.dict_curc[code] - self.dict_buy[code][3] * self.dict_set[f'주식{gubun}정정호가']
                else:
                    op = self.dict_curc[code] + self.dict_sell[code][3] * self.dict_set[f'주식{gubun}정정호가']
                self.CreateOrder(f'{gubun}정정', code, name, op, mc, df['주문번호'].iloc[-1], now(), False, None)
    ...
    def SysExit(self):
        ...
        self.SaveDayData()
        self.kwmservQ.put(['tele', '주식 트레이더 종료'])
        qtest_qwait(10)
        self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 실행 알림 - 트레이더 종료']])
    ...
    def SaveDayData(self):
        if self.dict_intg['장운영상태'] != 1 and len(self.df_td) > 0:
            ...
            if len(df) == 0:
                ...
                self.kwmservQ.put(['query', ['거래디비', df, 's_totaltradelist', 'append']])
                if self.dict_set['주식알림소리']:
                    self.kwmservQ.put(['sound', '일별실현손익를 저장하였습니다.'])
                self.kwmservQ.put(['window', [ui_num['S로그텍스트'], '시스템 명령 실행 알림 - 일별실현손익 저장 완료']])
    ...
    def OnReceiveMsg(self, sScrNo, sRQName, sTrCode, sMsg):
        print(f'[{now()}]{sMsg}')
        self.kwmservQ.put(['window', [ui_num['S오더텍스트'], f'{sMsg}']])
        if '매수증거금' in sMsg:
            sn = int(sScrNo)
    ...
    def OperationAlert(self, current):
        if self.dict_set['주식알림소리']:
            if current == '084000':
                self.kwmservQ.put(['sound', '장시작 20분 전입니다.'])
            elif current == '085000':
                self.kwmservQ.put(['sound', '장시작 10분 전입니다.'])
            elif current == '085500':
                self.kwmservQ.put(['sound', '장시작 5분 전입니다.'])
            elif current == '085900':
                self.kwmservQ.put(['sound', '장시작 1분 전입니다.'])
            elif current == '085930':
                self.kwmservQ.put(['sound', '장시작 30초 전입니다.'])
            elif current == '085940':
                self.kwmservQ.put(['sound', '장시작 20초 전입니다.'])
            elif current == '085950':
                self.kwmservQ.put(['sound', '장시작 10초 전입니다.'])
            elif current == '090000':
                self.kwmservQ.put(['sound', f"{self.dict_strg['당일날짜'][:4]}년 {self.dict_strg['당일날짜'][4:6]}월 "
                                            f"{self.dict_strg['당일날짜'][6:]}일 장이 시작되었습니다."])
            elif current == '152000':
                self.kwmservQ.put(['sound', '장마감 10분 전입니다.'])
            elif current == '152500':
                self.kwmservQ.put(['sound', '장마감 5분 전입니다.'])
            elif current == '152900':
                self.kwmservQ.put(['sound', '장마감 1분 전입니다.'])
            elif current == '152930':
                self.kwmservQ.put(['sound', '장마감 30초 전입니다.'])
            elif current == '152940':
                self.kwmservQ.put(['sound', '장마감 20초 전입니다.'])
            elif current == '152950':
                self.kwmservQ.put(['sound', '장마감 10초 전입니다.'])
            elif current == '153000':
                self.kwmservQ.put(['sound', f"{self.dict_strg['당일날짜'][:4]}년 {self.dict_strg['당일날짜'][4:6]}월 "
    ...
    def OnReceiveChejanData(self, gubun, itemcnt, fidlist):
        ...
        if gubun == '0':
            try:
                ...
                self.kwmservQ.put(['window', [ui_num['S로그텍스트'], f'시스템 명령 오류 알림 - OnReceiveChejanData 0 {e}']])
            else:
                try:
    ...
    def UpdateChejanData(self, code, name, sc, ot, og, oc, cc, mc, op, cp, ct, on, oon, sn):
        ...
        if ot == '접수' and mc > 0:
            if og == '매수':
                self.dict_buy[code] = [timedelta_sec(self.dict_set['주식매수취소시간초']), 0, op, GetHogaunit(code in self.list_kosd, op, self.int_hgtime)]
                if '지정가' in self.dict_set['주식매수주문구분']:
                    self.dict_intg['추정예수금'] -= oc * op
                else:
                    self.dict_intg['추정예수금'] -= oc * sc
            elif og == '매도':
                self.dict_sell[code] = [timedelta_sec(self.dict_set['주식매도취소시간초']), 0, op, GetHogaunit(code in self.list_kosd, op, self.int_hgtime)]
            elif og == '매수정정':
                fix_count = self.dict_buy[code][1] + 1
                self.dict_buy[code] = [timedelta_sec(self.dict_set['주식매수취소시간초']), fix_count, op, GetHogaunit(code in self.list_kosd, op, self.int_hgtime)]
                ...
                self.dict_sell[code] = [timedelta_sec(self.dict_set['주식매도취소시간초']), fix_count, op, GetHogaunit(code in self.list_kosd, op, self.int_hgtime)]
                df = self.df_cj[self.df_cj['주문번호'] == oon]
                if len(df) > 0:
            ...
            self.kwmservQ.put(['window', [ui_num['S로그텍스트'], f'주문 관리 시스템 알림 - [{ot}] {name} | {op} | {oc} | {og}']])
            ...
            self.kwmservQ.put(['query', ['거래디비', self.df_jg, 's_jangolist', 'replace']])
            if self.dict_set['주식알림소리']:
            ...
            if self.dict_set['주식알림소리']:
                self.kwmservQ.put(['sound', f'{name} {oc}주를 {og}하였습니다'])
            self.kwmservQ.put(['window', [ui_num['S로그텍스트'], f'주문 관리 시스템 알림 - [{ot}] {name} | {op} | {oc} | {og}']])
    ...
    def UpdateTradelist(self, index, name, jg, pg, cc, sp, sg, ct):
        ...
        self.kwmservQ.put(['query', ['거래디비', df, 's_tradelist', 'append']])
    ...
    def UpdateTotaltradelist(self, first=False):
        ...
        if not first:
            self.kwmservQ.put(['tele', f'거래횟수 {tdt}회 / 총매수금액 {int(tbg):,}원 / 총매도금액 {int(tsg):,}원 / 총수익금액 {int(sig):,}원 / '
    ...
    def UpdateChegeollist(self, index, code, name, og, oc, cc, mc, cp, ct, op, on):
        ...
        self.kwmservQ.put(['query', ['거래디비', df, 's_chegeollist', 'append']])
    ...
    def OrderTimeLog(self, signal_time):
        ...
        self.kwmservQ.put(['window', [ui_num['S단순텍스트'], f'시그널 주문 시간 알림 - 발생시간과 주문시간의 차이는 [{gap:.6f}]초입니다.']])

    def GetOrderCodeList(self):
        return list(self.dict_buy.keys()) + list(self.dict_sell.keys())
