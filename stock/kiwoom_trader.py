...
class KiwoomTrader:
    ...
    def CheckOrder(self, data):
        ...
        if manual:
            ...
            if self.dict_set['주식매수금지거래횟수'] and self.dict_set['주식매수금지거래횟수값'] <= len(self.df_td[self.df_td['종목명'] == name].drop_duplicates('체결시간')):
                ...
                cancel = True
            elif NIJ and inthms < self.dict_set['주식전략종료시간'] and len(self.df_jg) >= self.dict_set['주식최대매수종목수']:
                cancel = True
    ...
    def CreateOrder(self, gubun, code, name, op, oc, on, signal_time, manual, ordertype):
        ...
        if manual:
            ...
            if self.dict_set['주식매수주문구분'] in ('지정가', '지정가IOC', '지정가FOK'):
                op += GetHogaunit(code in self.tuple_kosd, op, self.int_hgtime) * self.dict_set['주식매수지정가호가번호']
            ...
            if self.dict_set['주식매도주문구분'] in ('지정가', '지정가IOC', '지정가FOK'):
                op += GetHogaunit(code in self.tuple_kosd, op, self.int_hgtime) * self.dict_set['주식매도지정가호가번호']
    ...
    def OrderTimeControl(self, code=None):
        if code is None:
            ...
            for code, order_info in self.dict_buy.items():
                ...
                # [timedelta_sec(self.dict_set['주식매수취소시간초']), fix_count, op, GetHogaunit(code in self.tuple_kosd, op, self.int_hgtime)]
                elif order_info[1] < self.dict_set['주식매수정정횟수'] and code in self.dict_curc.keys() and \
            ...
            for code, order_info in self.dict_sell.items():
                ...
                elif order_info[1] < self.dict_set['주식매도정정횟수'] and code in self.dict_curc.keys() and \
            ...
            if code in self.dict_buy.keys():
                buy_cancel = False
                buy_modify = False
                order_info = self.dict_buy[code]
                if self.dict_set['주식매수취소시간'] and now() > order_info[0]:
                    buy_cancel = True
                elif order_info[1] < self.dict_set['주식매수정정횟수'] and code in self.dict_curc.keys() and \
                        self.dict_curc[code] >= order_info[2] + order_info[3] * self.dict_set['주식매수정정호가차이']:
                    buy_modify = True
                if buy_cancel:   self.CancelOrder(code, '매수')
                elif buy_modify: self.ModifyOrder(code, '매수')

            if code in self.dict_sell.keys():
                sell_cancel = False
                sell_modify = False
                order_info  = self.dict_sell[code]
                if self.dict_set['주식매도취소시간'] and now() > order_info[0]:
                    sell_cancel = True
                elif order_info[1] < self.dict_set['주식매도정정횟수'] and code in self.dict_curc.keys() and \
                        self.dict_curc[code] <= order_info[2] - order_info[3] * self.dict_set['주식매도정정호가차이']:
                    sell_modify = True
                if sell_cancel:   self.CancelOrder(code, '매도')
                elif sell_modify: self.ModifyOrder(code, '매도')
    ...
    def UpdateTotaljango(self):
        ...
        if self.dict_set['주식투자금고정']:
            tujagm = int(self.dict_set['주식투자금'] * 1_000_000)
        else:
            if '시장가' in self.dict_set['주식매수주문구분']:
                tujagm = int((self.dict_intg['추정예탁자산'] - self.dict_intg['추정예탁자산'] / self.dict_set['주식최대매수종목수'] * 0.3) / self.dict_set['주식최대매수종목수'])
            else:
                tujagm = int(self.dict_intg['추정예탁자산'] * 0.98 / self.dict_set['주식최대매수종목수'])
