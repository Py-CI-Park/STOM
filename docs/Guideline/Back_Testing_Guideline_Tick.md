...
5. **[필수] 범위 함수 괄호 사용**
   ...
   \- ✅ 올바름: `이동평균(30)`, `최고현재가(20, 1)`, `체결강도평균(10)`
   \- ❌ 잘못됨: `이동평균30`, `최고현재가20`, `체결강도평균10`
...
이동평균값 = 이동평균30          # ❌ CheckFactor 검증 실패
최고가 = 최고현재가20            # ❌ CheckFactor 검증 실패
체결강도 = 체결강도평균10        # ❌ CheckFactor 검증 실패
```
...
**✅ 올바른 예시**:
```python
self.vars[1] = [[6.0, 10.0, 1.0], 8.0]   # 정상 형식
self.vars[2] = [[1.5, 3.0, 0.5], 2.0]    # 정상 형식
self.vars[0] = [[30], 30]                # 고정값 형식
```
...
```python
# 시작 < 끝인데 간격이 음수 → 검증 실패
self.vars[1] = [[1.0, 10.0, -1.0], 5.0]   # ❌ 간격 부호 오류
self.vars[2] = [[5, 20, -5], 10]          # ❌ 간격 부호 오류

# 시작 > 끝인데 간격이 양수 → 검증 실패
self.vars[3] = [[10.0, 1.0, 1.0], 5.0]    # ❌ 간격 부호 오류
self.vars[4] = [[20, 5, 5], 10]           # ❌ 간격 부호 오류
```
