...
12. **self.vars 인덱스 구간 준수**:
    - `self.vars[0]`: 고정값 전용
    - `self.vars[1~24]`: 매수 변수 (연속 사용)
    - `self.vars[25~49]`: 매도 변수 (연속 사용)
    - 각 구간 내에서 인덱스 건너뛰기 금지 (2.7절 참조)
...
1. **고정값**: `self.vars[0]` - 평균 틱수 등 상수값
2. **매수 변수**: `self.vars[1]` ~ `self.vars[24]` - 최대 24개
3. **매도 변수**: `self.vars[25]` ~ `self.vars[49]` - 최대 25개
4. **통합 최적화(OR)**: 매수/매도 인덱스 혼합 사용 가능
...
# 매도 변수 (25~49)
self.vars[25] = [[28.0, 30.0, 0.5], 29.5]   # 매도 변수 1
self.vars[26] = [[-3.0, -1.0, 0.5], -2.0]   # 매도 변수 2
self.vars[27] = [[4.0, 6.0, 0.5], 5.0]      # 매도 변수 3
# ... (최대 49번까지)
```

```python
# ❌ 잘못된 사용 - 매수 구간 내 인덱스 건너뛰기
self.vars[0] = [[30], 30]
self.vars[1] = [[6.0, 10.0, 1.0], 8.0]
self.vars[3] = [[3.5, 5.0, 0.5], 4.0]       # ❌ self.vars[2]가 없음
self.vars[7] = [[5, 10, 1], 7]              # ❌ self.vars[4], [5], [6]이 없음

# ❌ 잘못된 사용 - 매도 구간을 1~24 내에 배치
self.vars[1] = [[6.0, 10.0, 1.0], 8.0]      # 매수
self.vars[2] = [[1.5, 3.0, 0.5], 2.0]       # 매수
self.vars[3] = [[28.0, 30.0, 0.5], 29.5]    # ❌ 매도를 매수 구간에 넣으면 안 됨
```

**왜 25번부터인가?**
- 매수 변수 최대 24개를 확보하기 위함 (1~24)
- 매수/매도 구간을 명확히 구분하여 가독성 향상
- 나중에 매수 변수를 추가해도 매도 변수 인덱스 변경 불필요
- STOM 백테스팅 시스템에서 권장하는 표준 구간 분리 방식

**인덱스 연속성 규칙**:
- **매수 구간 (1~24) 내에서** 연속으로 사용: `1, 2, 3, 4, 5, ...`
- **매도 구간 (25~49) 내에서** 연속으로 사용: `25, 26, 27, 28, ...`
- 각 구간 내에서 인덱스를 건너뛰면 안 됨
...
# 매도 변수: 25~27 (연속)
self.vars[25] = [[28.0, 30.0, 0.5], 29.5]   # 등락율 상한가
self.vars[26] = [[-3.0, -1.0, 0.5], -2.0]   # 손실 수익률
self.vars[27] = [[4.0, 6.0, 0.5], 5.0]      # 수익률 상한

# ✅ 이 경우 self.vars[6]~[24], [28]~[49]는 정의하지 않음
```
...
1. **매수 변수 검증**:
   ```python
   # 사용된 매수 인덱스 찾기
   매수_인덱스 = [1, 2, 3, 4, 5]  # 실제 사용한 인덱스

   # 연속성 확인
   for i in range(len(매수_인덱스) - 1):
       if 매수_인덱스[i+1] - 매수_인덱스[i] != 1:
           print(f"❌ 오류: {매수_인덱스[i]}와 {매수_인덱스[i+1]} 사이 건너뛰기 발견")
   ```

2. **매도 변수 검증**:
   ```python
   # 사용된 매도 인덱스 찾기
   매도_인덱스 = [25, 26, 27]  # 실제 사용한 인덱스

   # 연속성 확인
   for i in range(len(매도_인덱스) - 1):
       if 매도_인덱스[i+1] - 매도_인덱스[i] != 1:
           print(f"❌ 오류: {매도_인덱스[i]}와 {매도_인덱스[i+1]} 사이 건너뛰기 발견")
   ```

3. **구간 검증**:
   ```python
   # 모든 매수 변수가 1~24 구간에 있는지 확인
   for idx in 매수_인덱스:
       if not (1 <= idx <= 24):
           print(f"❌ 오류: 매수 변수 {idx}가 허용 구간(1~24)을 벗어남")

   # 모든 매도 변수가 25~49 구간에 있는지 확인
   for idx in 매도_인덱스:
       if not (25 <= idx <= 49):
           print(f"❌ 오류: 매도 변수 {idx}가 허용 구간(25~49)을 벗어남")
   ```
...
- [ ] **공통 계산 지표가 조건식/최적화 양쪽에 동일하게 정의**되어 있는지 확인
  ...
  ```
- [ ] **매수/매도 변수 인덱스가 올바른 구간에 배치**되었는지 확인
  - 매수 변수: `self.vars[1]` ~ `self.vars[24]` 구간 사용
  - 매도 변수: `self.vars[25]` ~ `self.vars[49]` 구간 사용
  - 각 구간 내에서 **연속적으로** 사용 (건너뛰기 금지)
  ...
  **검증 방법**:
  ...
  for idx in 매수_인덱스:
      if not (1 <= idx <= 24):
          print(f"❌ 매수 변수 {idx}가 허용 구간(1~24)을 벗어남")

  # 2) 매수 연속성 검증
  for i in range(len(매수_인덱스) - 1):
      if 매수_인덱스[i+1] - 매수_인덱스[i] != 1:
          print(f"❌ 매수 인덱스 {매수_인덱스[i]}와 {매수_인덱스[i+1]} 사이 건너뛰기")

  # 3) 매도 구간 검증 (25~49)
  for idx in 매도_인덱스:
      if not (25 <= idx <= 49):
          print(f"❌ 매도 변수 {idx}가 허용 구간(25~49)을 벗어남")

  # 4) 매도 연속성 검증
  for i in range(len(매도_인덱스) - 1):
      if 매도_인덱스[i+1] - 매도_인덱스[i] != 1:
          print(f"❌ 매도 인덱스 {매도_인덱스[i]}와 {매도_인덱스[i+1]} 사이 건너뛰기")
  ```
