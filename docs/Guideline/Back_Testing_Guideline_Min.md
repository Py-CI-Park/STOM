...
19. **매수5호가잔량합 (int)**  
    \- 매수 1~5호가 잔량 합  
    \- `매수5호가잔량합` : 현재 매수5호가잔량합  
    \- `매수5호가잔량합N(1)` : 1분봉 전의 매수5호가잔량합  
    ```python
    if 매수5호가잔량합 > 1000000 and 매수5호가잔량합 > 매수5호가잔량합N(1):
        매수 = True
    ```
...
6. **[필수] 범위 함수 괄호 사용**
   \- **모든 범위 함수는 반드시 괄호와 함께 사용해야 합니다** (`back_code_test.py` CheckFactor 검증)
   \- ✅ 올바름: `이동평균(20)`, `최고분봉고가(30, 1)`, `분당거래대금평균(60)`
   \- ❌ 잘못됨: `이동평균20`, `최고분봉고가30`, `분당거래대금평균60`
   \- 검증 대상 함수: 이동평균, 최고현재가, 최저현재가, 초당거래대금평균, 체결강도평균, 최고체결강도, 최저체결강도, 누적초당매수수량, 누적초당매도수량, 최고초당매수수량, 최고초당매도수량, 당일거래대금각도, 전일비각도, 분당거래대금평균, 누적분당매수수량, 누적분당매도수량, 최고분당매수수량, 최고분당매도수량, 최고분봉고가, 최저분봉저가

7. **[필수] self.vars 검증 규칙**
   \- 형식: `[[시작값, 끝값, 간격], 초기값]` 준수
   \- 범위 값 개수: 각 변수당 최대 20개 이하 (`(끝값 - 시작값) / 간격 + 1 ≤ 20`)
   \- 간격 부호: 시작값 < 끝값이면 양수, 시작값 > 끝값이면 음수
   \- ✅ 올바름: `[[20, 60, 10], 40]` (5개 생성)
   \- ❌ 잘못됨: `[[1, 100, 1], 50]` (100개 생성, 20개 초과)

---

## 4.5 조건식 검증 규칙 상세 (back_code_test.py 기반)

**🚨 이 섹션은 `backtester/back_code_test.py`의 검증 로직에 기반한 필수 준수 사항입니다:**

STOM 백테스트 엔진은 조건식을 실행하기 전에 자동으로 코드를 검증합니다. 검증에 실패하면 백테스트가 실행되지 않으므로, 아래 규칙을 반드시 준수해야 합니다.

### 4.5.1 범위 함수 괄호 사용 규칙 (CheckFactor 검증)
...
```python
# 단일 인자: 범위값만 지정
이동평균값 = 이동평균(20)             # 20분봉 이동평균
최고가 = 최고분봉고가(30)            # 30분봉 중 최고가
거래대금 = 분당거래대금평균(60)      # 60분봉 평균 거래대금

# 두 인자: 범위값 + 오프셋
이동평균값 = 이동평균(20, 1)         # 20분봉 이동평균, 1분봉 전
최고가 = 최고분봉고가(30, 2)         # 30분봉 중 최고가, 2분봉 전
```

**❌ 잘못된 사용 예시 (검증 실패)**:
```python
# 괄호 없이 숫자만 붙이는 경우 - 오류 발생
이동평균값 = 이동평균20              # ❌ CheckFactor 검증 실패
최고가 = 최고분봉고가30              # ❌ CheckFactor 검증 실패
거래대금 = 분당거래대금평균60        # ❌ CheckFactor 검증 실패
```

**검증 실패 시 출력 메시지**:
```
이동평균(20), 이동평균(20, 1) 형태로 사용하십시오.
최고분봉고가(30), 최고분봉고가(30, 1) 형태로 사용하십시오.
분당거래대금평균(60), 분당거래대금평균(60, 1) 형태로 사용하십시오.
```

### 4.5.2 self.vars 형식 검증

**올바른 형식**: `[[시작값, 끝값, 간격], 초기값]`

**✅ 올바른 예시**:
```python
self.vars[1] = [[20, 60, 10], 40]      # 정상 형식 (분봉 구간)
self.vars[2] = [[30, 70, 10], 50]      # 정상 형식 (RSI 임계값)
self.vars[0] = [[60], 60]              # 고정값 형식
```

**❌ 잘못된 예시 (검증 실패)**:
```python
self.vars[1] = [[20, 60], 40]          # ❌ 간격 누락
self.vars[2] = [30, 70, 10]            # ❌ 중첩 리스트 누락
self.vars[3] = [[30, 70, 10, 50]]      # ❌ 초기값 분리 안됨
```
...
### 4.5.3 범위 값 개수 제한 (최대 20개)

**값 개수 계산 공식**: `값_개수 = (끝값 - 시작값) / 간격 + 1`

**✅ 올바른 범위 설정**:
```python
# 값 개수: (60 - 20) / 10 + 1 = 5개 ✅
self.vars[1] = [[20, 60, 10], 40]

# 값 개수: (70 - 30) / 10 + 1 = 5개 ✅
self.vars[2] = [[30, 70, 10], 50]
...
# ✅ 해결 방법 1: 간격을 5로 증가 → 20개 생성
self.vars[1] = [[1, 100, 5], 50]

# ✅ 해결 방법 2: 범위를 축소 → 11개 생성
self.vars[1] = [[40, 60, 2], 50]
```

### 4.5.4 간격 부호 검증

**규칙**: 시작값과 끝값의 대소 관계에 따라 간격 부호가 올바른지 검증

**✅ 올바른 간격 부호**:
```python
# 시작 < 끝 → 간격은 양수
self.vars[1] = [[20, 60, 10], 40]       # ✅ 20 < 60, 간격 +10
self.vars[2] = [[30, 70, 10], 50]       # ✅ 30 < 70, 간격 +10

# 시작 > 끝 → 간격은 음수
self.vars[3] = [[60, 20, -10], 40]      # ✅ 60 > 20, 간격 -10
self.vars[4] = [[70, 30, -10], 50]      # ✅ 70 > 30, 간격 -10
```

**❌ 잘못된 간격 부호**:
```python
# 시작 < 끝인데 간격이 음수 → 검증 실패
self.vars[1] = [[20, 60, -10], 40]      # ❌ 간격 부호 오류
self.vars[2] = [[30, 70, -10], 50]      # ❌ 간격 부호 오류

# 시작 > 끝인데 간격이 양수 → 검증 실패
self.vars[3] = [[60, 20, 10], 40]       # ❌ 간격 부호 오류
self.vars[4] = [[70, 30, 10], 50]       # ❌ 간격 부호 오류
```

**검증 실패 시 출력 메시지**:
```
self.vars[1]의 범위 간격 부호 오류
→ 시작값(20) < 끝값(60)인데 간격이 음수(-10)입니다
→ 간격을 양수로 변경하세요: [[20, 60, 10], 40]
```
...
- [ ] **범위 함수 괄호 사용**: 모든 범위 함수를 `함수명(인자)` 형태로 사용
  ```python
  # ✅ 올바름: 이동평균(20), 최고분봉고가(30, 1)
  # ❌ 잘못됨: 이동평균20, 최고분봉고가30
  ```
...
- [ ] **간격 부호**: 시작값과 끝값의 대소 관계에 맞는 간격 부호 사용
  ```python
  # ✅ 올바름: [[20, 60, 10], ...] (양수 간격)
  # ❌ 잘못됨: [[20, 60, -10], ...] (간격 부호 오류)
  ```
