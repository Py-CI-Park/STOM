...
??? backtest.py                      # ???? ?? ??
??? optimiz.py                       # ???? ???(Optuna)
??? optimiz_genetic_algorithm.py     # ?? ???? ???
??? optimiz_conditions.py            # ?? ???
??? rolling_walk_forward_test.py    # ???? (Rolling Walk-Forward)
??? backfinder.py                    # ???? ?? ??
??? back_static.py                   # ???? ?? API + ?? ???????
??? analysis/                        # ?? ?? ??
?   ??? plotting.py                  # ???/??
...
?   ??? results.py                   # ?? ??/??
?   ??? indicators.py                # ?? ??/?? ??
?   ??? optuna_server.py             # Optuna ???? ??
??? back_code_test.py                # ?? ?? ??
??? back_subtotal.py                 # ???? ??
??? backengine_kiwoom_tick.py        # ?? ????? (??1)
??? backengine_kiwoom_tick2.py       # ?? ????? (??2)
??? backengine_kiwoom_min.py         # ?? ?? ???? (??1)
??? backengine_kiwoom_min2.py        # ?? ?? ???? (??2)
??? backengine_upbit_tick.py         # ??? ????? (??1)
??? backengine_upbit_tick2.py        # ??? ????? (??2)
??? backengine_upbit_min.py          # ??? ?? ???? (??1)
??? backengine_upbit_min2.py         # ??? ?? ???? (??2)
??? backengine_binance_tick.py       # ???? ????? (??1)
??? backengine_binance_tick2.py      # ???? ????? (??2)
??? backengine_binance_min.py        # ???? ?? ???? (??1)
??? backengine_binance_min2.py       # ???? ?? ???? (??2)
```
