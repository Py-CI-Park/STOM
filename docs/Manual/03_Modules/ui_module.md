...
├── ui_button_clicked_*.py        # 버튼 이벤트 핸들러
│   ├── ui_button_clicked_s1.py   # 주식 거래 탭
│   ├── ui_button_clicked_s2.py   # 주식 백테스트 탭
│   ├── ui_button_clicked_c1.py   # 코인 거래 탭
│   ├── ui_button_clicked_c2.py   # 코인 백테스트 탭
│   └── ui_button_clicked_*.py    # 기타 탭
├── ui_update_tablewidget.py      # 테이블 업데이트
├── ui_draw_chart.py              # 차트 그리기 (matplotlib)
├── ui_draw_realchart.py          # 실시간 차트 (pyqtgraph)
├── set_*.py                      # UI 컴포넌트 설정
│   ├── set_style.py              # 스타일 시트
│   ├── set_window.py             # 윈도우 설정
│   ├── set_stock_tab.py          # 주식 탭 설정
│   ├── set_coin_tab.py           # 코인 탭 설정
│   └── set_backtest_tab.py       # 백테스트 탭 설정
└── dialog/
    ├── dialog_strategy.py        # 전략 설정 다이얼로그
    ├── dialog_kimp.py            # 김프 모니터링 다이얼로그
    └── dialog_setting.py         # 시스템 설정 다이얼로그
```
