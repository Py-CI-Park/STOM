...
- **파일명 규칙**: `{save_file_name}_{suffix}.csv`
  ...
  - `{suffix}`는 아래 표의 `_detail`, `_summary`, `_filter`, `_enhanced_detail` 등
- **인코딩**: `utf-8-sig` (Excel에서 한글/기호 깨짐 최소화를 위해 BOM 포함)
- **구분자/개행**: `,` / `\n` (기본 pandas `to_csv` 출력)
- **인덱스 컬럼 포함 여부**
  - `*_detail.csv`, `*_enhanced_detail.csv`: `index=True` → **첫 번째 컬럼이 DataFrame 인덱스**
  - 그 외 CSV: `index=False`
...
- `detail.csv`/`enhanced_detail.csv`는 위 원본 스키마 뒤에 파생지표 컬럼이 추가됩니다.
...
| 파일명 | 내용 | 행 수(대략) |
|--------|------|-------------|
| `{전략명}_enhanced_detail.csv` | 전체 거래 상세 + 강화 파생지표(최대 27개) | 거래 수 |
| `{전략명}_filter_analysis.csv` | 강화된 단일 필터 분석(통계 포함) | 필터 수 |
| `{전략명}_optimal_thresholds.csv` | 컬럼별 동적 최적 임계값 탐색 결과 | 컬럼 수 |
| `{전략명}_filter_combinations.csv` | 2~3개 필터 조합 시너지 분석 | 조합 수 |
| `{전략명}_filter_stability.csv` | 기간 분할 기반 필터 안정성 | 주요 필터 수 |

#### 10.3.1 `{전략명}_enhanced_detail.csv` 컬럼 참고

`enhanced_detail.csv`는 `detail.csv`의 컬럼 구성에 더해, `CalculateEnhancedDerivedMetrics()`의 파생지표가 추가됩니다.

- 기본 파생지표(14개)는 `10.2.1` 참고
- 강화 파생지표(추가분)는 `2.3 전체 파생 지표 목록`의 15~27번 참고

#### 10.3.2 `{전략명}_filter_analysis.csv` 컬럼 정의

`filter_analysis.csv`는 `AnalyzeFilterEffectsEnhanced()` 결과를 그대로 CSV로 저장한 것으로, “개선 효과 + 통계적 유의성 + 효과 크기”까지 함께 제공합니다.
...
| `조합유형` | `2개 조합` 또는 `3개 조합` |
| `필터1/필터2/필터3` | 조합된 필터명 |
| `개별개선합` | 개별 필터 수익개선금액의 합 |
| `조합개선` | OR 조건으로 조합했을 때의 개선 금액 |
| `시너지효과` | `조합개선 - 개별개선합` |
| `시너지비율` | `시너지효과 / 개별개선합 × 100` |
| `제외비율` | 조합 적용 시 제외 비율(%) |
| `잔여승률` | 조합 적용 후 잔여 거래 승률(%) |
| `잔여거래수` | 잔여 거래 수 |
| `권장` | 시너지 비율 기반 간단 등급 |
