# 텔레그램 추가 차트 시스템 분석 문서 (v2.6)

**문서 버전**: v2.6 (라벨 단위 축약/알림 순서 정리/히트맵 합계·시총 라벨)
**작성일**: 2025-12-10
**최종 업데이트**: 2025-12-28
**관련 커밋**: `48932dcd371e38b95ec1cac5be3c6ec00268db54` (STOM_V1 머지), `1fef8950f4fcb035699a7c6379fe074ab70d7206` (v2.3 ML 모델 실행별 저장/재현), `a9a253dbca070ad803695f9c1b1a0326876a3d7b` (만 단위 라벨/히트맵 합계), `a63c6f9aa09260c5810dc70a234c1a7392ef7e2d` (보유금액 일자 오버플로우 방지), `a2fc7cc7d9298b23924dd58d6f1ab5275ecb1da3` (로그축 마이너스 글리프 보정), `bba3de33f3a7dfbc88fe05f0b6af87a4c7d0c4f3` (알림 순서/세그먼트 히트맵 라벨)
...
2. **텔레그램 추가 차트 3종 역할 분리**
   ...
   - `*_comparison.png`: **매수/매도 시점 상세 비교 차트** (x축 tick 세분화, 매수 위험도 점수별 수익금 분포, 매수시간×시가총액 3D 히트맵(평균 수익률), 보유시간 분 단위 산점도 포함)
...
### 1.4 v2.2 추가 개선 사항 (필터/정합성/진단)

1. **매수 시점 전체 변수 스캔 + 범위(하한~상한) 필터 탐색 추가**
   - `AnalyzeFilterEffectsEnhanced()`에서 **매수 시점에 알 수 있는 모든 숫자형 컬럼**을 후보로 스캔
   - 단순 `이상/미만`뿐 아니라 `하한~상한 범위(inside/outside)` 필터도 함께 탐색
   - 룩어헤드 방지: `매도*`, `*변화*`, `보유시간`, `수익*` 등 결과 확정 컬럼은 자동 제외
2. **필터 적용 누적 개선을 “동시 적용(중복 반영)” 기준으로 정합화**
   - `GenerateFilterCode(..., df_tsg=...)`에서 그리디 방식으로 필터를 조합해
     “단순합(naive)”과 “동시 적용(combined)”의 차이(중복/상쇄)를 함께 표시
3. **사후진단(룩어헤드 포함) 필터 분석 CSV/차트 추가**
   - 실거래 적용 금지(룩어헤드) 전제의 “사후 진단용” 분석을 별도 제공
   - `{전략명}_filter_lookahead.csv` 생성 + `{전략명}_enhanced.png` Chart 17에 Top 15 표시
4. **단위 정합성 강화**
   - 거래대금: **백만**(raw) → 라벨 표시: 억/조
   - 시가총액: **억**(raw) → 1조 이상은 조 단위 라벨

### 1.4.1 v2.3 추가 개선 사항 (ML 모델 실행별 저장/재현)

1. **전략키(조건식 해시) 기반 모델 저장**
   - 매수/매도 전략 코드(전체 텍스트)를 정규화한 뒤 sha256으로 `strategy_key` 생성
   - 동일 조건식이면 실행명이 달라도 같은 전략 폴더에 모델이 누적 저장됨
2. **실행별(run) 저장 + 전략별 latest 갱신**
   - 실행마다 `backtester/models/strategies/{strategy_key}/runs/`에 `*_ml_bundle.joblib`/`*_meta.json` 저장
   - 동시에 `latest_ml_bundle.joblib`/`latest_ml_bundle_meta.json`을 갱신하여 “가장 최근 모델”을 즉시 재사용 가능
3. **재현 모드(load_latest/load_only) 지원**
   - 기본값은 `train`이며, 매 백테스팅 실행마다 새로 학습(더 좋은 모델 탐색) 후 저장
   - `ml_train_mode='load_latest'`로 실행하면 학습 없이 저장된 최신 모델을 로드하여 동일 입력 데이터에 대해 동일 `_ML` 값을 재현
   - 피처 스키마 해시(`feature_schema_hash`)로 입력 컬럼 호환성(스키마 변경)을 진단
4. **리포트(txt)에 모델 정보/경로/규칙 덤프**
   - AUC/F1/BA, 손실비율, 피처수, `strategy_key`, `feature_schema_hash`, 저장 경로(run/latest/meta) 기록
   - 얕은 트리 규칙(참고용) 및 “매수매도위험도” 회귀 예측 요약을 함께 출력
5. **실거래/필터 적용 시 주의**
   - `train` 모드는 학습 시점/모델 선택에 따라 `_ML` 값이 달라질 수 있으므로,
     동일한 필터 값을 유지하려면 `load_latest`(또는 특정 run 번들 경로 고정)를 사용 권장
   - 자동 재학습 정책(주기/성능 기준)은 추후 확장 대상으로 유지
...
└── strategies/
    └── {strategy_key}/
        ├── strategy_code.txt
        ├── latest_ml_bundle.joblib
        ├── latest_ml_bundle_meta.json
        ├── runs_index.jsonl
        └── runs/
            ├── {save_file_name}_{run_id}_{YYYYMMDDhhmmss}_ml_bundle.joblib
            └── {save_file_name}_{run_id}_{YYYYMMDDhhmmss}_ml_bundle_meta.json
```

- `strategy_code.txt`: 전략키를 만든 매수/매도 조건식 원문(최초 1회 저장)
- `latest_*`: 해당 전략키의 최신 번들/메타(재현에 주로 사용)
- `runs/*`: 실행별 번들/메타 누적 보관
- `runs_index.jsonl`: 실행 로그(번들 버전/생성시각/피처 스키마 해시/저장 경로 등)

### 1.4.2 v2.4 추가 개선 사항 (조건식 출력/필터 적용 미리보기/Chart 18)

1. **텔레그램에 매수/매도 조건식(요약) 전송**
   - 강화 분석 메시지에 `if 매수:`/`if 매도:` 블록 일부를 함께 전송(공부/검증 목적)
2. **강화 차트에 조건식(요약) 포함**
   - `{전략명}_enhanced.png`의 요약 텍스트 영역에 전략키/조건식 일부 표시
3. **자동 생성 필터 조합 적용 미리보기 차트 2종 추가**
   - `{전략명}_filtered_.png`: 단계별 누적개선/제외비율 + 시간/요일 분포 비교
   - `{전략명}_filtered.png`: 필터 적용 수익곡선 요약(누적/이익·손실 바)
4. **강화 차트 Chart 18 추가**
   - 자동 생성 코드(combine_steps) 기준으로 “필터를 조합할수록 예상 수익금이 어떻게 개선되는지” 시각화
...
### 1.4.4 v2.6 추가 개선 사항 (라벨 축약/알림 정리/가독성 개선)

1. **필터 분포 차트 라벨을 “만” 단위로 축약**
   - 시간대별/요일별 수익금 막대 라벨을 `+1,059만` 형태로 표시
   - 양/음수 모두 반영하며, 길이가 긴 수치를 가독성 중심으로 축약(반올림)
2. **텔레그램 사전 알림 순서 정리**
   - 차트 생성 전 `완료` → `분석/차트 생성 중` → `이후 단계 요약` 메시지를 먼저 전송
   - 긴 분석 단계 진행 상황을 사전에 안내하여 체감 대기시간 감소
3. **로그축 마이너스 라벨 폰트 경고 방지**
   - symlog 축 라벨에서 유니코드 마이너스 대신 ASCII `-`를 사용
   - `Font 'default' does not have a glyph for U+2212` 경고 제거
4. **보유금액 일자축 오버플로우 방지**
   - 일자 집계 인덱스 캐스팅을 `int64`로 고정하여 `6xxxxxxx` 형태 깨짐 방지
5. **세그먼트 히트맵 보조 정보 강화**
   - 시가총액 구간 라벨(소형/중형/대형주 범위) 표시
   - 기준/필터 히트맵에 `Profit 합계` 메모 박스 추가
...
│  • 이미지 3: {전략명}_enhanced.png     (강화된 분석 - 18개 차트) NEW │
│  • 텍스트: 통계적 유의 필터 추천                                     │
│  • 텍스트: 조합 필터 추천                                            │
│  • 텍스트: 자동 생성 조건식 코드                                     │
...
#### 매수 시점 위험도 점수 (LOOKAHEAD-FREE)

`위험도점수`는 **매수 시점에 알 수 있는 정보만** 사용하여 0~100점으로 계산합니다.  
(매도 시점 확정 지표/변화량/보유시간 등은 포함하지 않습니다.)

```python
# 예시(요약): 과열/약세/비유동/변동성/호가 불리 등을 점수화
df['위험도점수'] = 0
df.loc[df['매수등락율'] >= 20, '위험도점수'] += 20
df.loc[df['매수체결강도'] < 80, '위험도점수'] += 15
df.loc[df['시가총액'] < 1000, '위험도점수'] += 15
df.loc[df['매수스프레드'] >= 0.5, '위험도점수'] += 10
df['위험도점수'] = df['위험도점수'].clip(0, 100)
```

**해석**:
- 점수가 높을수록 “진입(매수) 시점” 기준으로 리스크가 큰 케이스(과열/비유동/약세/변동성 등)
- `filter.csv`의 `매수 위험도점수 N점 이상 제외`는 **룩어헤드 없이** “매수 안 하기(진입 회피)” 필터로 해석 가능

#### 매수/매도 위험도 점수 (사후 진단용 / LOOKAHEAD 포함)

`매수매도위험도점수`는 **매도 시점 확정 정보(매도-매수 변화량 등)** 를 포함하여 0~100점으로 계산합니다.

- 목적: 손실 거래에서 어떤 변화량/신호가 같이 발생했는지 **진단/비교(차트용)**  
- 주의: 매수 시점에 알 수 없는 정보가 포함되므로, **필터 추천/조건식 자동 생성에는 사용하지 않습니다.**

```python
# 예시(요약): 매도-매수 변화량 기반 "사후 위험도" 점수화
df['매수매도위험도점수'] = 0
...
| 14 | 위험도점수 | 0-100점 | **매수 시점 기반 종합 위험 점수(룩어헤드 제거)** |
| 15 | 매수매도위험도점수 | 0-100점 | 매도-매수 변화 기반 위험 점수(사후 진단용/룩어헤드) |
| 16 | **이익여부** | 불리언 | `수익금 > 0` (ML 타겟 변수) |
| 17 | **모멘텀점수** | 연속값 | 등락율+체결강도 정규화 |
| 18 | **매수변동폭** | 연속값 | 매수 시점 고가-저가 |
| 19 | **매수변동폭비율** | % | (매수고가-매수저가)/매수가 × 100 |
| 20 | **매도변동폭비율** | % | (매도고가-매도저가)/매도가 × 100 |
| 21 | **변동성변화** | % | 매도변동폭비율 - 매수변동폭비율 |
| 22 | **스프레드영향** | 연속값 | 매수스프레드가 수익률에 미치는 패널티(정규화) |
| 23 | **시간대평균수익률** | % | 동일 매수시(시간대) 평균 수익률 |
| 24 | **타이밍점수** | 연속값 | 시간대평균수익률을 Z-score로 정규화 |
| 25 | **리스크조정수익률** | % | 수익률 / (위험요소 가중치) |
| 26 | **거래품질점수** | 0-100점 | 체결강도/거래대금/스프레드 등 종합 품질 |
| 27 | **연속이익** | 정수 | 직전 거래까지의 연속 이익 횟수 |
| 28 | **연속손실** | 정수 | 직전 거래까지의 연속 손실 횟수 |

> **중요(룩어헤드 방지 원칙)**  
> 필터 추천/조건식 자동 생성은 **매수 시점에 알 수 있는 정보만** 사용합니다.  
> - 제외(사용 금지): `매도*` 컬럼, `*변화`(매도-매수), `매수매도위험도점수`, `보유시간`, `급락신호` 등 **매도 시점 확정 정보**  
> - 사용 가능: `매수*` 컬럼(등락율/체결강도/거래대금/호가/스프레드 등), `시가총액`, `위험도점수(매수 기반)` 등  
> 매도-매수 변화량 지표는 **진단/비교 차트용**으로만 남겨두고, 필터 분석에는 사용하지 않습니다.
...
columns_config = [
    ...
    # 파생 지표(매수 시점 기반) - 존재할 때만 추가
    ('위험도점수', 'greater', '매수위험도 {:.0f}점 이상 제외'),
    ('거래품질점수', 'less', '거래품질(매수) {:.0f}점 미만 제외'),
    ('모멘텀점수', 'less', '모멘텀(매수) {:.1f} 미만 제외'),
]
```
...
```csv
column,direction,optimal_threshold,임계값(원본),임계값(표시),원본단위,improvement,excluded_ratio,efficiency
매수등락율,greater,22.5,22.5,,,1250000,8.5,147059
매수체결강도,less,85,85,,,890000,6.2,143548
매수당일거래대금,less,10000,10000,100억,백만,720000,5.8,124138
```
...
2개 조합,매수등락율 25% 이상 제외,매수체결강도 80 미만 제외,1250000,1580000,330000,26.4%,★★★
2개 조합,시간대 14시 제외,매수 위험도점수 50점 이상 제외,890000,920000,30000,3.4%,★★
```
...
#### 6.2.0 중요도 해석/주의사항(꼭 읽기)

- `feature_importances_`는 Decision Tree가 분기하면서 감소시킨 **불순도(Gini) 감소량**을 누적해 정규화한 값입니다.
- 값이 크다고 해서 “인과”를 의미하지 않습니다. (상관/중복 변수에서는 중요도가 분산되거나 특정 변수로 쏠릴 수 있음)
- 본 시스템에서 ML 중요도는 “예측 모델”을 만들기 위한 목적이 아니라, **필터/임계값 탐색 우선순위 힌트**로 사용합니다.
- `model_accuracy`는 `train/test` 분리 기준이며, `baseline_accuracy(다수 클래스)`보다 의미 있게 높지 않으면 과대해석하지 않습니다.
...
시가총액,매수시가총액 < 1000,120000,180000,3,35.2,불안정
```
...
def GenerateFilterCode(filter_results, df_tsg=None, top_n=5):
    ...
    Returns:
        ...
        combine_steps: 동시 적용(중복 반영) 기준 적용 순서/누적 개선
        summary: 총 개선(동시적용 vs 단순합), 중복/상쇄 등 요약
    """
```
...
### 8.3 조합 방식(AND)과 누적 개선(중복 반영) 해석

필터 분석의 “제외 조건”은 **매수를 안 하는(진입 회피) 조건**입니다.  
따라서 실제 전략 코드에 붙일 때는 “제외”의 반대인 **유지(keep) 조건**을 `AND`로 결합합니다.

- **단일 필터**: `A 이상 제외` → 실제 적용은 `A < ...` 를 AND로 추가(= 해당 구간만 매수)
- **다중 필터**: 각 필터의 제외 대상이 겹칠 수 있어, `수익개선금액`의 **단순 합산(naive)** 은 과대평가될 수 있음
- (v2.2) `GenerateFilterCode(..., df_tsg=...)`는 실제 `df_tsg`에 조건식을 적용해 **동시 적용(중복 반영)** 기준으로
  누적 개선과 `overlap_loss(단순합-동시적용)`를 함께 계산/표시합니다.

예시(요약):

```
총 3개 필터 (조합: AND)
예상 총 개선(동시 적용/중복 반영): 2,860,000원
개별개선합(단순 합산): 3,200,000원, 중복/상쇄: -340,000원
적용 순서(추가개선→누적개선, 누적제외%):
  1. ... : +800,000 → 누적 +800,000 (제외 6.2%)
  2. ... : +500,000 → 누적 +1,300,000 (제외 10.1%)
```
...
| gs[6,:] | 필터 조합 적용 시 수익금 변화 | 자동 생성 필터 조합 단계별 예상 수익금 변화 + 누적 제외비율 (NEW) |
| gs[7,:] | 사후진단(룩어헤드) Top 15 | 매도 확정 정보 포함 필터 효과(진단용, 실거래 적용 금지) |
...
- **매수 시점 초당(틱) 스냅샷(3개)**: `매수초당매수수량`, `매수초당매도수량`, `매수초당거래대금`
...
- **매도 시점 초당(틱) 스냅샷(3개)**: `매도초당매수수량`, `매도초당매도수량`, `매도초당거래대금`
...
```csv
종목명,시가총액,매수시간,매도시간,보유시간,매수가,매도가,매수금액,매도금액,수익률,수익금,수익금합계,매도조건,추가매수시간,매수일자,매수시,매수분,매수초,매수등락율,매수시가등락율,매수당일거래대금,매수체결강도,매수전일비,매수회전율,매수전일동시간비,매수고가,매수저가,매수고저평균대비등락율,매수매도총잔량,매수매수총잔량,매수호가잔량비,매수매도호가1,매수매수호가1,매수스프레드,매수초당매수수량,매수초당매도수량,매수초당거래대금,매도등락율,매도시가등락율,매도당일거래대금,매도체결강도,매도전일비,매도회전율,매도전일동시간비,매도고가,매도저가,매도고저평균대비등락율,매도매도총잔량,매도매수총잔량,매도호가잔량비,매도매도호가1,매도매수호가1,매도스프레드,매도초당매수수량,매도초당매도수량,매도초당거래대금
```
...
- **사후(매도-매수) 파생지표(진단/비교용)**: `등락율변화`, `체결강도변화`, `전일비변화`, `회전율변화`, `호가잔량비변화`, `거래대금변화율`, `체결강도변화율`, `등락추세`, `체결강도추세`, `거래량추세`, `급락신호`, `매도세증가`, `거래량급감`
- **매수 시점 기반 파생지표(룩어헤드 없음 / 필터 추천 가능)**: `위험도점수`, `모멘텀점수`, `매수변동폭`, `매수변동폭비율`, `거래품질점수` 등
  - 매도 시점 컬럼이 모두 존재하지 않으면(`매도등락율` 등 6개) **사후 파생지표**는 계산되지 않지만, **매수 기반 파생지표**는 계산됩니다.
...
| 컬럼 | 의미 |
|------|------|
| `분류` | 필터 카테고리(예: `시간대`, `등락율`, `체결강도`, `거래대금`, `시가총액`, `호가`, `위험신호`, `품질`) |
| `필터명` | 필터 설명(예: `매수등락율 25% 이상 제외`) |
...
| `{전략명}_filter_lookahead.csv` | (진단용) 룩어헤드 포함 필터 효과 | 사후 분석용(실거래 적용 금지) |
...
#### 10.3.2A `{전략명}_filter_lookahead.csv` (사후진단/룩어헤드) 참고

`filter_lookahead.csv`는 `AnalyzeFilterEffectsLookahead()` 결과이며, 컬럼 스키마는 `{전략명}_filter.csv`(강화 모드)와 동일합니다.

- 매도 시점 확정 정보/변화량/보유시간 등 **룩어헤드가 포함될 수 있음**
- `적용권장` 컬럼에 `※진단용(룩어헤드)`로 표기
- 실거래/자동 생성 조건식에는 사용하지 말고, “사후에 어떤 신호가 손실과 같이 나왔는지” 점검용으로만 사용
...
- detail.csv 컬럼 설명/공식: 주요 파생지표/원본값의 의미, 단위, 계산식(가능한 항목)

### 10.6 detail.csv 컬럼 순서 (v2.4)

detail.csv는 저장 시 `backtester/detail_schema.py`의 `reorder_detail_columns()`를 통해 가독성 중심으로 컬럼을 재정렬합니다.

- 1) 일반/식별/시간: `종목명`, `시가총액`/`포지션`, `매수시간`, `매도시간`, `보유시간`, `매도조건`, `추가매수시간`
- 2) 가격/금액/성과: `매수가`, `매도가`, `매수금액`, `매도금액`, `수익률`, `수익금`, `수익금합계`
- 3) 매수 시점 스냅샷: `매수*`
- 4) 매도 시점 스냅샷: `매도*`
- 5) 핵심 파생/리스크/ML: `모멘텀점수`, `거래품질점수`, `위험도점수`, `손실확률_ML`, `위험도_ML`, `예측매수매도위험도점수_ML`, `리스크조정수익률` 등
- 6) 기타/차트 보조: `수익금합계020`~`수익금합계480`, `수익금합계{0..29}`, `이익금액`, `손실금액` 등(맨 뒤)
...
1. **텍스트**: `"{전략명} {날짜} 완료."`
2. **텍스트**: `"{전략명} {날짜} 분석/차트 생성 중... (거래 N회)"`
3. **텍스트**: 이후 전송/생성 단계 요약 (NEW)
4. **텍스트**: 매수/매도 조건식(요약) (있을 때)
5. **텍스트**: 백테스트 진행 로그(있을 때)
6. **이미지**: `{전략명}_.png` (부가정보 차트)
7. **이미지**: `{전략명}.png` (수익곡선 차트)
8. **이미지**: `{전략명}_analysis.png` (백테스팅 결과 분석 차트)
9. **이미지**: `{전략명}_comparison.png` (매수/매도 시점 상세 비교 차트)
10. **이미지**: `{전략명}_enhanced.png` (필터 기능 분석 차트 - 18개)
11. **텍스트**: ML 위험도 예측 결과(AUC/F1/BA + 소요 시간 + 신뢰도 PASS/FAIL)
12. **텍스트**: 필터 추천/조합 추천/안정성 요약
13. **텍스트**: 자동 생성 조건식 요약
14. **텍스트**: 필터 적용 미리보기 요약
15. **이미지**: `{전략명}_filtered_.png` (필터 적용 분포/단계 요약)
16. **이미지**: `{전략명}_filtered.png` (필터 적용 수익곡선 요약)
...
### 11.3 필터 적용 미리보기 (v2.4)

`GenerateFilterCode()`로 만들어진 “유지(keep) 조건 AND 조합”을 detail DataFrame에 적용하여, **필터 적용 후 성과를 빠르게 확인**하기 위한 보조 차트를 추가 전송합니다.

- `{전략명}_filtered_.png`: 단계별 누적개선/제외비율 + 시간/요일 분포(기준 vs 필터)
  - 시간/요일 막대 라벨은 `만` 단위로 축약 표시(반올림)하여 가독성을 확보
- `{전략명}_filtered.png`: 필터 적용 누적 수익금/이익·손실 바 + 조건식/필터 요약 텍스트

**주의(해석 범위)**:
- 이 미리보기는 “거래 리스트(df_tsg)를 조건으로 필터링”한 결과이며, 포지션 사이징/동시 보유/재진입 등 엔진 레벨의 모든 동작을 재시뮬레이션하지는 않습니다.
- 실제 적용 검증은 생성된 조건식을 전략 코드에 반영 후 **재백테스팅**으로 확인합니다.
...
| `back_analysis_enhanced.py` | `PltEnhancedAnalysisCharts()` | 18개 강화 차트 |
...
아래 용어들은 텔레그램 메시지/리포트에서 자주 등장하는 핵심 지표들입니다.

### 분류(Classification) 지표

- **AUC(ROC-AUC)**: ROC 곡선(거짓양성률 FPR vs 참양성률 TPR) 아래 면적. 0.5는 랜덤 수준, 1.0은 완전 분리.
- **F1**: `2 * (Precision * Recall) / (Precision + Recall)` (정밀도와 재현율의 조화평균).
- **Precision(정밀도)**: `TP / (TP + FP)` (양성으로 예측한 것 중 실제 양성 비율).
- **Recall(재현율, TPR)**: `TP / (TP + FN)` (실제 양성 중 모델이 맞춘 비율).
- **Confusion Matrix**: (TP, FP, TN, FN)로 예측 성능을 요약하는 표.
- **임계값(Threshold)**: 확률 출력(예: 손실확률)에서 “양성/음성”을 가르는 기준(기본 0.5지만 목적에 따라 조정).

### 회귀(Regression) 지표

- **MAE**: `mean(|y - ŷ|)` (평균 절대 오차). 낮을수록 좋음.
- **R2**: 결정계수. 1에 가까울수록 좋고, 0이면 평균 예측과 유사, 음수면 평균보다도 못함.
- **상관(Correlation)**: 두 값의 선형 관계 강도(절대값이 클수록 강함). 0에 가까우면 선형 관계가 약함.

### 모델/학습 용어

- **Ensemble**: 여러 모델의 예측을 결합(평균/가중/스태킹 등)해 성능/안정성을 높이는 방식.
- **RandomForest**: 여러 결정트리를 랜덤하게 학습해 평균내는 앙상블.
- **GradientBoosting**: 이전 모델의 오차를 보완하도록 순차적으로 약한 학습기를 쌓는 부스팅.
- **Feature Importance(특성 중요도)**: 모델이 어떤 입력 변수(피처)를 얼마나 사용했는지의 상대적 기여도(트리 기반은 Gini 중요도 등).
- **Train/Test**: 학습(train)과 평가(test)를 분리해 일반화 성능을 측정.
- **Overfitting(과적합)**: 학습 데이터에만 잘 맞고 새로운 데이터에서 성능이 떨어지는 현상.
...
| v2.1 | 2025-12-14 | CSV 중복 제거/차트 역할 분리/리포트(txt) 추가 |
| v2.2 | 2025-12-13 | 필터 범위 탐색/중복 반영(동시 적용) 정합화/사후진단(룩어헤드) 추가 |
| v2.3 | 2025-12-19 | ML 모델 실행별 저장/재현 추가 |
| v2.4 | 2025-12-19 | 조건식 출력/필터 적용 미리보기/Chart 18/detail.csv 컬럼 순서 개선 |
| v2.5 | 2025-12-20 | ML 시간/신뢰도 게이트/리포트·스터디 문서 강화 |
| v2.6 | 2025-12-28 | 라벨 만 단위 축약/알림 순서 정리/히트맵 보조 정보 추가 |
