# 텔레그램 추가 차트 시스템 분석 문서 (v2.3)

**문서 버전**: v2.3 (ML 모델 실행별 저장/재현 추가)
**작성일**: 2025-12-10
**최종 업데이트**: 2025-12-19
...
│  • 이미지 3: {전략명}_enhanced.png     (강화된 분석 - 17개 차트) NEW │
│  • 텍스트: 통계적 유의 필터 추천                                     │
│  • 텍스트: 조합 필터 추천                                            │
│  • 텍스트: 자동 생성 조건식 코드                                     │
...
| gs[6,:] | 사후진단(룩어헤드) Top 15 | 매도 확정 정보 포함 필터 효과(진단용, 실거래 적용 금지) |

### 9.2 출력 파일

```
파일: backtester/graph/{전략명}_enhanced.png
크기: 20x34 인치 (2400x4080 픽셀)
해상도: 120 DPI
```
...
6. **이미지**: `{전략명}_enhanced.png` (필터 기능 분석 차트 - 17개)
...
| `back_analysis_enhanced.py` | `PltEnhancedAnalysisCharts()` | 17개 강화 차트 |
