...
2. **텔레그램 추가 차트 3종 역할 분리**
   - `*_analysis.png`: **백테스팅 결과 분석 차트** (매수 시각 X축을 `HH:MM` 기반으로 동적 표시)
   - `*_comparison.png`: **매수/매도 시점 상세 비교 차트** (변화량/추세/보유시간 등)
   - `*_enhanced.png`: **필터 기능 분석 차트** (통계/시너지/안정성/임계값/코드 생성 중심)
3. **산출물 리포트(txt) 자동 생성**
