...
### 9.1 Option 1: 백테스팅 엔진에 "당일 재매수 차단" 옵션 추가 (⭐ 권장)

#### 개념

세그먼트 필터의 **본래 의도**를 백테스팅 엔진에 정확히 구현합니다. 필터에 의해 차단된 종목은 해당 일에 재매수를 금지하는 옵션을 추가합니다.

이 방안이 권장되는 이유:
1. **필터 의도와 실행이 일치**: 세그먼트 분석에서 "제외"로 판정한 종목은 실제로 당일 전체에서 제외됨
2. **예측 = 실제**: combos.csv의 예측값과 실제 백테스트 결과가 정확히 일치
3. **사용자 옵션화**: 하위 호환성 유지, 기존 동작도 선택 가능
...
```python
# back_static.py 또는 backengine_stock_min.py에 추가
class SegmentFilterManager:
    """세그먼트 필터 당일 차단 종목 관리"""
    
    def __init__(self):
        # 당일 필터 차단 종목: {(종목코드, 날짜): 차단시간}
        self.blocked_stock_dates = {}
        
        # 옵션: True면 당일 재매수 차단
        self.prevent_same_day_reentry = True
    
    def is_blocked_today(self, 종목코드: str, 날짜: int) -> bool:
        """해당 종목이 오늘 이미 차단되었는지 확인"""
        return (종목코드, 날짜) in self.blocked_stock_dates
    
    def mark_as_blocked(self, 종목코드: str, 날짜: int, 시간: int):
        """종목을 당일 차단 목록에 추가"""
        self.blocked_stock_dates[(종목코드, 날짜)] = 시간
    
    def clear_for_new_day(self, 새날짜: int):
        """새 날짜 시작 시 차단 목록 초기화"""
        self.blocked_stock_dates = {
            k: v for k, v in self.blocked_stock_dates.items() 
            if k[1] == 새날짜
        }
```
...
```python
# backengine_stock_min.py 수정 예시
class BackEngine:
    def __init__(self, ...):
        # 기존 초기화 코드...
        
        # 세그먼트 필터 매니저 추가
        self.segment_filter_manager = SegmentFilterManager()
        
        # 옵션 읽기 (dict_set에서 또는 기본값)
        self.segment_filter_manager.prevent_same_day_reentry = \
            self.dict_set.get('세그먼트필터_당일재매수차단', True)
    
    def evaluate_buy_condition(self, 종목코드, 현재날짜, 현재시간, ...):
        """매수 조건 평가 (세그먼트 필터 포함)"""
        
        # Step 1: 당일 이미 차단된 종목인지 확인
        if self.segment_filter_manager.prevent_same_day_reentry:
            if self.segment_filter_manager.is_blocked_today(종목코드, 현재날짜):
                return False  # 당일 재매수 차단
        
        # Step 2: 기존 매수 조건 평가
        매수조건충족 = self.evaluate_existing_conditions(...)
        
        if not 매수조건충족:
            return False
        
        # Step 3: 세그먼트 필터 평가
        if self.is_segment_filter_active():
            필터통과 = self.evaluate_segment_filter(종목코드, 현재시간, ...)
            
            if not 필터통과:
                # 필터 차단 → 당일 재매수 금지 목록에 추가
                self.segment_filter_manager.mark_as_blocked(
                    종목코드, 현재날짜, 현재시간
                )
                return False
    ...
    def on_new_day(self, 새날짜):
        """새 날짜 시작 시 호출"""
        # 전날 차단 목록 정리
        self.segment_filter_manager.clear_for_new_day(새날짜)
```

##### 9.1.3 UI 설정 옵션 추가

```python
# utility/setting.py 또는 UI 설정에 추가
DEFAULT_SETTINGS = {
    # ... 기존 설정 ...
    '세그먼트필터_당일재매수차단': True,  # 기본값: 활성화
}
```
...
[Option 1 적용 후 동작]
09:32 - 종목A 매수 조건 충족 → T1+소형주 → 필터 차단!
        → blocked_stock_dates에 (종목A, 20250407) 추가
10:00 - 종목A 매수 조건 충족 → is_blocked_today() → True → 스킵!
11:02 - 종목A 매수 조건 충족 → is_blocked_today() → True → 스킵!
(이후 당일 내내 종목A는 매수되지 않음)

결과: 0건 거래 ← 세그먼트 분석 예측과 정확히 일치!
```

#### 장점
- ✅ 세그먼트 분석 예측 = 실제 결과 (정확히 일치)
- ✅ 필터의 본래 의도대로 동작
- ✅ 옵션 OFF 시 기존 동작 유지 (하위 호환성)
- ✅ 사용자가 동작을 명시적으로 선택 가능
- ✅ 대체 매수로 인한 추가 손실 방지

#### 단점
- ⚠️ 백테스팅 엔진 수정 필요 (핵심 로직 변경)
- ⚠️ 충분한 테스트 필요
- ⚠️ 일부 수익 기회 차단 가능 (의도된 동작)
...
### 9.2 Option 2: 세그먼트 분석 시 대체 매수 시뮬레이션

#### 개념

세그먼트 분석 단계에서 필터 적용 후 동일 종목의 이후 매수 가능성을 시뮬레이션합니다. 현재 백테스팅 엔진의 동작을 그대로 유지하면서, 분석 예측의 정확도를 높이는 방안입니다.

#### 구현 방법

```python
# segment_analyzer.py에 추가
def simulate_with_substitution_buy(original_trades: pd.DataFrame, 
                                    segment_filter_fn) -> pd.DataFrame:
    """
    대체 매수를 고려한 세그먼트 필터 시뮬레이션
    
    Args:
        original_trades: 원본 거래 목록 (detail.csv)
        segment_filter_fn: 세그먼트 필터 함수
    
    Returns:
        필터 적용 후 예상 거래 목록 (대체 매수 포함)
    """
    remaining_trades = []
...
- ⚠️ 세그먼트 분석 로직 복잡도 증가
- ⚠️ 매도 시간 정보 필요
- ⚠️ 근본적인 필터 의도와 실행의 불일치 해소 안 됨

#### 구현 난이도: ⭐⭐⭐ (중간)
#### 권장 우선순위: 3순위 (Option 1이 불가능할 경우)

---

### 9.3 Option 3: 종목-일 단위 필터 적용

#### 개념

세그먼트 분석 시점에서, 특정 시점에 필터로 차단된 종목은 해당 일 전체의 모든 거래를 차단합니다.

#### 구현 방법

```python
# segment_analyzer.py에 추가
def filter_by_stock_date(original_trades: pd.DataFrame,
                          segment_filter_fn) -> pd.DataFrame:
    """
    종목+일자 단위 필터링
    
    특정 시점에 필터 차단 → 해당 종목의 당일 전체 거래 제외
    """
    # Step 1: 차단할 종목+일자 식별
    blocked_stock_dates = set()
    
    for idx, trade in original_trades.iterrows():
        if segment_filter_fn(trade):
            # 이 거래가 필터에 의해 차단됨
            blocked_stock_dates.add((trade['종목명'], trade['매수일자']))
    
    # Step 2: 차단된 종목+일자의 모든 거래 제외
    filtered_trades = original_trades[
        ~original_trades.apply(
            lambda x: (x['종목명'], x['매수일자']) in blocked_stock_dates,
            axis=1
        )
    ]
...
근본적인 해결이 아닌 단기 완화책으로, 예측값에 불확실성 범위를 함께 표시합니다.

#### 구현 방법

```python
# combos.csv 출력 시 수정
def calculate_uncertainty_range(optimistic_count: int, 
                                 historical_ratio: float = 1.97) -> dict:
    """
    대체 매수를 고려한 불확실성 범위 계산
    
    Args:
        optimistic_count: 낙관적 예측 (대체 매수 없다고 가정)
        historical_ratio: 과거 관측된 실제/예측 비율 (기본값: 1.97 = 2955/1497)
    """
    pessimistic_count = int(optimistic_count * historical_ratio)
    expected_count = int((optimistic_count + pessimistic_count) / 2)
    
    return {
        '예상_하한': optimistic_count,
        '예상_상한': pessimistic_count,
        '예상_중앙': expected_count,
        '불확실성_비율': historical_ratio
    }

# 출력 예시
"""
예상 잔여 거래: 1,497건 ~ 2,948건 (대체 매수 포함 시)
  - 낙관적 예측: 1,497건 (대체 매수 없다고 가정)
  - 비관적 예측: 2,948건 (과거 비율 기반)
  - 중앙값: 2,223건
  
⚠️ 주의: 실제 거래 수는 대체 매수로 인해 예측보다 많아질 수 있습니다.
"""
```
...
#### 구현 난이도: ⭐ (매우 쉬움)
#### 권장 우선순위: 2순위 (즉시 적용 가능한 단기 조치)

---

### 9.5 Option 5: 세그먼트 필터 코드에 종목별 당일 차단 로직 삽입 (신규)

#### 개념

segment_code_final.txt에 자동 생성되는 필터 코드에 "종목별 당일 차단" 로직을 포함시킵니다. 백테스팅 엔진 자체를 수정하지 않고, 조건식 코드 레벨에서 해결합니다.

#### 구현 방법

```python
# 현재 segment_code_final.txt 구조
"""
# 세그먼트 필터
if 시가총액 >= 6432 and 92900 <= 시분초 < 95900:  # 대형주_T1
    if 매수고저평균대비등락율 >= 3.43:
        매수 = False
"""

# 개선된 segment_code_final.txt 구조
"""
# === 당일 필터 차단 종목 관리 (자동 생성) ===
if '당일차단종목' not in dir():
    당일차단종목 = {}

현재날짜 = int(str(매수시간)[:8])  # YYYYMMDD

# 이미 차단된 종목인지 확인
if (종목코드, 현재날짜) in 당일차단종목:
    매수 = False
else:
    # 세그먼트 필터 적용
    필터차단 = False
...
#### 단점
- ⚠️ 조건식 코드 복잡도 증가
- ⚠️ 전역 변수(`당일차단종목`) 사용 필요
- ⚠️ 코드 자동 생성 로직 수정 필요

#### 구현 난이도: ⭐⭐⭐ (중간)
#### 권장 우선순위: 5순위 (Option 1 대안)

---

### 9.6 해결 방안 비교 요약

| 옵션 | 방법 | 정확도 향상 | 구현 난이도 | 호환성 | 권장 우선순위 |
|------|------|------------|------------|--------|:------------:|
| **Option 1** | 백테스팅 엔진 당일 재매수 차단 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | **1순위 ⭐** |
| **Option 4** | 불확실성 범위 표시 | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 2순위 (단기) |
| **Option 2** | 대체 매수 시뮬레이션 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 3순위 |
| **Option 3** | 종목-일 단위 필터 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | 4순위 |
| **Option 5** | 조건식 코드에 로직 삽입 | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | 5순위 |

### 9.7 권장 구현 로드맵

#### 즉시 (1일 이내)
- **Option 4 적용**: 세그먼트 분석 결과에 불확실성 범위 및 경고 메시지 표시
- 사용자 문서에 대체 매수 현상 설명 추가

#### 단기 (1-2주)
- **Option 1 구현**: 백테스팅 엔진에 `세그먼트필터_당일재매수차단` 옵션 추가
- 기존 동작(옵션 OFF)을 기본값으로 유지하여 하위 호환성 확보
- 충분한 테스트 후 기본값을 ON으로 변경 검토

#### 중기 (선택)
- 필요시 Option 2 (대체 매수 시뮬레이션) 추가 구현
- 두 방식의 결과 비교 검증 도구 개발
...
#### 10.1.3 이것은 버그가 아니지만 개선이 필요함
- 모든 시스템이 **현재 설계대로** 동작
- 문제는 **필터의 본래 의도**(당일 전체 차단)와 **현재 구현**(시점별 차단)의 차이
- **권장 해결책**: 백테스팅 엔진에 "당일 재매수 차단" 옵션 추가 (Option 1)
...
#### 단기 (1-2주) ⭐ 핵심
- **Option 1 적용 (권장)**: 백테스팅 엔진에 `세그먼트필터_당일재매수차단` 옵션 추가
  - 필터의 본래 의도(당일 전체 차단)를 정확히 구현
  - 기존 동작(옵션 OFF)을 기본값으로 유지하여 하위 호환성 확보
  - 충분한 테스트 후 기본값을 ON으로 변경 검토

#### 중기 (선택)
- **Option 2 적용**: 필요시 세그먼트 분석에 대체 매수 시뮬레이션 로직 추가
- 두 방식(Option 1 vs Option 2)의 결과 비교 검증 도구 개발
