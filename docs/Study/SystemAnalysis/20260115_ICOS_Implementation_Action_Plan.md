...
- [x] 기본 지표만 사용하는 조건식으로 ICOS 정상 실행 ✅
- [ ] 거래가 1건 이상 발생하는 것 확인 (실제 백테스트로 검증 필요)
- [ ] 반복 사이클에서 조건식 개선 관찰 (실제 백테스트로 검증 필요)
- [x] 모든 BackEngine 지표가 SyncBacktestRunner에서 지원됨 ✅

### 3.2 품질 기준

- [ ] 동일 조건식에서 BackEngine과 SyncBacktestRunner 결과 비교 (실제 데이터로 검증 필요)
- [ ] 오차율 5% 이내 (허용 오차: 부동소수점 차이)
- [x] 예외 발생 시 명확한 에러 메시지 출력 ✅

---

## 4. 구현 체크리스트

- [x] Phase 1: 기본 지표 함수 30개 추가 ✅ (2026-01-15 완료)
- [x] Phase 2: 고급 분석 함수 10개 추가 ✅ (2026-01-15 완료)
- [x] Phase 3: 예외 처리 개선 ✅ (2026-01-15 완료)
- [x] Phase 4: 통합 테스트 ✅ (2026-01-15 완료)
...
**기본 지표 (인덱스 8-44)**:
- 거래대금증감N(8), 전일비N(9), 회전율N(10), 전일동시간비N(11)
- 시가총액N(12), 라운드피겨위5호가이내N(13)
- 초당매수수량N(14), 초당매도수량N(15), 초당거래대금N(19)
- 고저평균대비등락율N(20), 매도총잔량N(21), 매수총잔량N(22)
- 5단계 호가 (23-32), 5단계 잔량 (33-42)
- 매도수5호가잔량합N(43), 관심종목N(44)
...
- NameError 발생 시 미지원 함수명 로깅
- verbose 모드에서 상세 오류 메시지 출력
- 동일 오류 중복 로깅 방지 (set으로 관리)
