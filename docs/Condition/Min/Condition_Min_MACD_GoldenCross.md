...
MACD_0선위치여부     = (MACD > 0)                                    # 개선1: MACD 0선 확인
히스토그램_양수여부  = (MACDH > 0)
히스토그램_증가여부  = (MACDH > MACDH_N(1) and MACDH_N(1) > MACDH_N(2))  # 개선2: 히스토그램 증가 확인
RSI_중립여부         = (40 <= RSI <= 70)
이평위치여부         = (현재가 > 이동평균(20))
...
if not (관심종목 == 1):
    ...
    매수 = False
elif not MACD_0선위치여부:                    # 개선1: 추세 강화 확인
    매수 = False
elif not 히스토그램_양수여부:
    매수 = False
elif not 히스토그램_증가여부:                 # 개선2: 모멘텀 가속 확인
    매수 = False
elif not RSI_중립여부:
    매수 = False
elif not 이평위치여부:
...
### 매수·매도 통합 최적화 범위 - C_M_MGC_OR_Min
...
# 총 경우의 수: 3 × 3 × 3 × 3 × 3 × 2 × 3 × 3 × 3 = 39,366
```
...
1) **MACD 0선 확인** ✅
   - MACD가 0선 위에 위치할 때만 매수
   - 상승 추세 확인으로 신뢰도 향상
   - 구현 위치: 매수 조건식 77-78행, 최적화 조건식 161-162행

2) **히스토그램 증가 확인** ✅
   - 히스토그램이 연속 2봉 증가 확인
   - 모멘텀 가속 확인으로 진입 타이밍 개선
   - 구현 위치: 매수 조건식 81-82행, 최적화 조건식 165-166행
