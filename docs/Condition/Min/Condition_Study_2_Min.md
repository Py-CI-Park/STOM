...
본 문서는 STOM 주식 자동거래 시스템에서 **분봉 기반 Study 2 전략 (09:10 기준)**을 정의한다.

- **대상 시간 구간**: 09:10:00 이후
- **대상 종목**: 관심종목, 가격대 1,000원~30,000원, 등락율 1~20%
- **전략 타입**: 분봉 데이터 기반 조건식 탐색 (Minute Study 2)
- **핵심 변수**: 분당거래대금, 분당매수수량, 고저평균대비등락율, 체결강도, 호가정보
- **업데이트 이력**:
  - 초기 문서 작성 (틱 조건식에서 분봉 버전으로 변환)

# 조건식 찾기 - 분봉 버전 (Condition Study 2 91000 - Minute Version)
