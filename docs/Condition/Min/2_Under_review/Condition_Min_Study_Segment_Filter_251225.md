...
조건식 및 최적화에서 공통으로 사용되는 지표의 계산식입니다. 본 전략은 세그먼트 필터링 결과의 임계값을 정확히 적용하기 위해 내부 엔진(`metrics_enhanced.py`)의 계산 공식을 따릅니다.
...
매수호가잔량비 = (매수총잔량 / (매도총잔량 + 1e-6)) * 100 # metrics_enhanced.py 호환용
매수변동폭 = 고가 - 저가
...
# --------------------------------
# metrics_enhanced.py의 CalculateEnhancedDerivedMetrics 로직과 동기화
위험도점수 = 0
# 1) 등락율 과열
if 등락율 >= 20: 위험도점수 += 20
if 등락율 >= 25: 위험도점수 += 10
