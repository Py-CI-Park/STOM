...
self.vars[1] = [[4.0, 5.0, 6.0], 5.0]                   # 수익률 조건
self.vars[2] = [[-4.0, -3.0, -2.0], -4.0]               # 손실 수익률 조건
self.vars[3] = [[2.0, 3.0, 4.0], 3.0]                   # 최고수익률 기준값
self.vars[4] = [[0.6, 0.7, 0.8], 0.7]                   # 최고수익률 대비 현재 수익률 비율
self.vars[5] = [[5, 10, 15, 20], 10]                    # 보유시간(분) 조건
self.vars[6] = [[-1.0, 0.0, 1.0], 1.0]                  # 보유시간 초과 시 수익률 하한
self.vars[7] = [[-0.2, -0.15, -0.1, -0.05], -0.1]       # 현재가 변화율 하한
self.vars[8] = [[-2.0, -1.0, 0], -1.0]                  # 손실 수익률 조건
self.vars[9] = [[2.0, 3.0, 4.0], 3.0]                   # 최고수익률 조건
# 매도 총 경우의 수: (1 × 3 × 3 × 3 × 3 × 4 × 3 × 4 × 3 × 3 = 51,840)

# GA 최적화 범위 (매수 변수)
self.vars[10] = [[15.0, 17.5, 20.0], 15.0]              # 등락율 범위
self.vars[11] = [[1.0, 1.25, 1.5, 1.75, 2.0], 1.5]      # 분당거래대금 증가율
self.vars[12] = [[65, 70, 75], 70]                      # RSI 범위
self.vars[13] = [[91000, 91500, 92000], 91000]          # 시분초 범위
self.vars[14] = [[6.0, 8.0, 10.0], 10.0]                # 시가등락율 범위
self.vars[15] = [[7.0, 8.0, 9.0], 8.0]                  # 시가대비등락율 범위
self.vars[16] = [[10, 20, 30], 10]                      # 고가 대비 현재가 비율
self.vars[17] = [[5, 15, 25], 5]                        # 누적거래대금비율
self.vars[18] = [[10, 15, 20], 10]                      # 전일비 범위
self.vars[19] = [[0.5, 1.0, 1.5, 2.0], 1.0]             # 회전율 범위
self.vars[20] = [[0.5, 1.0, 1.5], 0.5]                  # 최고분당매수수량 대비 최고분당매도수량 비율
self.vars[21] = [[1.5, 2.5, 3.5], 1.5]                  # 분당거래대금 대비 분당거래대금평균 비율
self.vars[22] = [[20, 30, 40], 20]                      # 분당매수수량 대비 매도총잔량 비율
self.vars[23] = [[0.25, 0.375, 0.5], 0.5]               # 매수총잔량 대비 매도총잔량 비율
self.vars[24] = [[1.5, 2.0, 2.5], 1.5]                  # 매수잔량1 대비 매도잔량1 비율
self.vars[25] = [[1, 2, 3], 1]                          # 누적분당매수수량 대비 누적분당매도수량 비율
# 매수 총 경우의 수: (3 × 5 × 3 × 3 × 3 × 3 × 3 × 3 × 3 × 4 × 3 × 3 × 3 × 3 × 3 × 3 = 1,180,980,480)

# 매수 매도 총 경우의 수: (51,840 × 1,180,980,480 ≈ 6.12 × 10^13)
```
...
**우선순위: 높음**
- **09:05~09:10 시간대 특화 전략 개발**
  - 초반 모멘텀 이후 안정화 구간 포착
  - 각 시간대별 다른 RSI 임계값 적용
  - 구현 난이도: 낮음

**우선순위: 중간**
- **시간대별 거래대금 패턴 분석**
  - 시간대별 분당거래대금 평균 차이 반영
  - 구간별 최적 임계값 설정
  - 구현 난이도: 중
...
**우선순위: 높음**
- **분봉시가/고가/저가 활용 강화**
  - 현재 일봉 고가/저가 사용 → 분봉고가/분봉저가 우선 사용
  - 분봉 캔들 패턴 분석 (망치형, 역망치형, 도지 등)
  - 구현 난이도: 중

**우선순위: 중간**
- **이동평균 기간 최적화**
  - 현재 60분 이동평균 단독 사용 → 20분/60분 이중 이동평균 활용
  - 단기/중기 이동평균 골든크로스 확인
  - 구현 난이도: 낮음
...
#주식트레이딩 #분봉트레이딩 #매수조건 #매도조건 #최적화 #유전알고리즘 #파이썬 #트레이딩로직 #분봉백테스팅 #RSI #MACD #9010전략 #리스크관리

---

**작성일**: 2025-07-15
**최종 수정일**: 2025-11-21
**버전**: v1.2 (조건식 개선 방향 연구 섹션 추가)
