...
if not (관심종목 == 1):
    매수 = False
elif not (self.vars[1][1] < 현재가 <= self.vars[2][1]):    # 현재가 범위 최적화
    매수 = False
elif not (self.vars[3][1] < 등락율 <= self.vars[4][1]):     # 등락율 범위 최적화
    매수 = False
elif not (고저평균대비등락율 > 0):
    매수 = False
elif not (현재가 < VI아래5호가):
    매수 = False
elif not (초당거래대금 > 초당거래대금N(1) * self.vars[5][1]):  # 초당거래대금 급등 배수 최적화
    매수 = False
elif 라운드피겨위5호가이내:
    매수 = False
elif not (시분초 < 91000):
    매수 = False
elif not (초당순매수금액 > self.vars[6][1] and 초당순매수금액 < self.vars[7][1]):  # 초당순매수금액 범위 최적화
    매수 = False
elif not (전일비 > self.vars[8][1]):  # 전일비 최적화
    매수 = False
elif not (회전율 > self.vars[9][1]):  # 회전율 최적화
    매수 = False
elif not (당일거래대금 > self.vars[10][1]):  # 당일거래대금 최적화
    매수 = False
elif not (체결강도 >= self.vars[11][1]):  # 체결강도 최적화
    매수 = False
...
# 상한가 근접 조건
if 등락율 > self.vars[12][1]:  # 최대 등락율 최적화
    매도 = True
# 시간대별 수익 목표
elif 시분초 < 90500 and self.vars[13][1] <= 수익률:  # 초반 목표수익률 최적화
    매도 = True
elif 시분초 < 90500 and 수익률 <= self.vars[14][1]:  # 초반 손절수익률 최적화
    매도 = True
elif 시분초 < 91000 and self.vars[15][1] <= 수익률:  # 중반 목표수익률 최적화
    매도 = True
elif 시분초 < 91000 and 수익률 <= self.vars[16][1]:  # 중반 손절수익률 최적화
    매도 = True
# 일반 수익 실현
elif 수익률 >= self.vars[17][1]:  # 일반 목표수익률 최적화
    매도 = True
elif 수익률 <= self.vars[18][1]:  # 일반 손절수익률 최적화
    매도 = True
elif 최고수익률 > self.vars[19][1] and 수익률 < 최고수익률 * self.vars[20][1]:  # 고점 대비 하락 비율 최적화
    매도 = True
elif 보유시간 > self.vars[21][1]:  # 보유시간 제한 최적화
    매도 = True
...
# 매수 최적화 범위 (1~24) - 핵심 변수
self.vars[1] = [[800, 1000, 1200], 1000]           # 범위갯수: 3  # 현재가 하한
self.vars[2] = [[25000, 30000, 35000], 30000]      # 범위갯수: 3  # 현재가 상한
self.vars[3] = [[0.5, 1.0, 1.5], 1.0]              # 범위갯수: 3  # 등락율 하한
self.vars[4] = [[18.0, 20.0, 22.0], 20.0]          # 범위갯수: 3  # 등락율 상한
self.vars[5] = [[1.3, 1.5, 1.7], 1.5]              # 범위갯수: 3  # 초당거래대금 급등 배수
self.vars[11] = [[95, 105, 115], 105]              # 범위갯수: 3  # 체결강도

# 매도 최적화 범위 (12~21) - 핵심 변수
self.vars[12] = [[28.5, 29.5, 30.0], 29.5]         # 범위갯수: 3  # 최대 등락율
self.vars[17] = [[1.5, 2.0, 2.5], 2.0]             # 범위갯수: 3  # 일반 목표수익률
self.vars[18] = [[-2.5, -2.0, -1.5], -2.0]         # 범위갯수: 3  # 일반 손절수익률
self.vars[21] = [[1200, 1800, 2400], 1800]         # 범위갯수: 3  # 보유시간 제한(초)
...
# 매수 최적화 범위 (1~24)
self.vars[1] = [[800, 1200, 100], 1000]            # 범위갯수: 5  # 현재가 하한
self.vars[2] = [[25000, 35000, 2500], 30000]       # 범위갯수: 5  # 현재가 상한
self.vars[3] = [[0.5, 1.5, 0.25], 1.0]             # 범위갯수: 5  # 등락율 하한
self.vars[4] = [[18.0, 22.0, 1.0], 20.0]           # 범위갯수: 5  # 등락율 상한
self.vars[5] = [[1.3, 1.7, 0.1], 1.5]              # 범위갯수: 5  # 초당거래대금 급등 배수
self.vars[6] = [[20, 40, 5], 30]                   # 범위갯수: 5  # 초당순매수금액 하한
self.vars[7] = [[250, 350, 25], 300]               # 범위갯수: 5  # 초당순매수금액 상한
self.vars[8] = [[8, 12, 1], 10]                    # 범위갯수: 5  # 전일비
self.vars[9] = [[0.8, 1.2, 0.1], 1.0]              # 범위갯수: 5  # 회전율
self.vars[10] = [[200, 400, 50], 300]              # 범위갯수: 5  # 당일거래대금
self.vars[11] = [[95, 115, 5], 105]                # 범위갯수: 5  # 체결강도

# 매수 총 경우의 수: 5^11 = 48,828,125
```
...
```python
# 매도 최적화 범위 (12~21)
self.vars[12] = [[28.5, 30.0, 0.5], 29.5]          # 범위갯수: 4  # 최대 등락율
self.vars[13] = [[4.0, 6.0, 0.5], 5.0]             # 범위갯수: 5  # 초반(90500) 목표수익률
self.vars[14] = [[-5.0, -3.0, 0.5], -4.0]          # 범위갯수: 5  # 초반(90500) 손절수익률
self.vars[15] = [[4.0, 6.0, 0.5], 5.0]             # 범위갯수: 5  # 중반(91000) 목표수익률
self.vars[16] = [[-5.0, -3.0, 0.5], -4.0]          # 범위갯수: 5  # 중반(91000) 손절수익률
self.vars[17] = [[1.5, 2.5, 0.25], 2.0]            # 범위갯수: 5  # 일반 목표수익률
self.vars[18] = [[-2.5, -1.5, 0.25], -2.0]         # 범위갯수: 5  # 일반 손절수익률
self.vars[19] = [[2.0, 3.0, 0.25], 2.5]            # 범위갯수: 5  # 고점 기준 수익률
self.vars[20] = [[0.7, 0.9, 0.05], 0.8]            # 범위갯수: 5  # 고점 대비 하락 비율
self.vars[21] = [[1200, 2400, 300], 1800]          # 범위갯수: 5  # 보유시간 제한(초)

# 매도 총 경우의 수: 4 × 5^9 = 7,812,500
```
...
# 매수 최적화 범위 (GA용, 1~24)
self.vars[1] = [[800, 900, 1000, 1100, 1200], 1000]                            # 현재가 하한
self.vars[2] = [[25000, 27500, 30000, 32500, 35000], 30000]                    # 현재가 상한
self.vars[3] = [[0.5, 0.75, 1.0, 1.25, 1.5], 1.0]                              # 등락율 하한
self.vars[4] = [[18.0, 19.0, 20.0, 21.0, 22.0], 20.0]                          # 등락율 상한
self.vars[5] = [[1.3, 1.4, 1.5, 1.6, 1.7], 1.5]                                # 초당거래대금 급등 배수
self.vars[6] = [[20, 25, 30, 35, 40], 30]                                      # 초당순매수금액 하한
self.vars[7] = [[250, 275, 300, 325, 350], 300]                                # 초당순매수금액 상한
self.vars[8] = [[8, 9, 10, 11, 12], 10]                                        # 전일비
self.vars[9] = [[0.8, 0.9, 1.0, 1.1, 1.2], 1.0]                                # 회전율
self.vars[10] = [[200, 250, 300, 350, 400], 300]                               # 당일거래대금
self.vars[11] = [[95, 100, 105, 110, 115], 105]                                # 체결강도

# 매도 최적화 범위 (GA용, 12~21)
self.vars[12] = [[28.5, 29.0, 29.5, 30.0], 29.5]                               # 최대 등락율
self.vars[13] = [[4.0, 4.5, 5.0, 5.5, 6.0], 5.0]                               # 초반(90500) 목표수익률
self.vars[14] = [[-5.0, -4.5, -4.0, -3.5, -3.0], -4.0]                         # 초반(90500) 손절수익률
self.vars[15] = [[4.0, 4.5, 5.0, 5.5, 6.0], 5.0]                               # 중반(91000) 목표수익률
self.vars[16] = [[-5.0, -4.5, -4.0, -3.5, -3.0], -4.0]                         # 중반(91000) 손절수익률
self.vars[17] = [[1.5, 1.75, 2.0, 2.25, 2.5], 2.0]                             # 일반 목표수익률
self.vars[18] = [[-2.5, -2.25, -2.0, -1.75, -1.5], -2.0]                       # 일반 손절수익률
self.vars[19] = [[2.0, 2.25, 2.5, 2.75, 3.0], 2.5]                             # 고점 기준 수익률
self.vars[20] = [[0.7, 0.75, 0.8, 0.85, 0.9], 0.8]                             # 고점 대비 하락 비율
self.vars[21] = [[1200, 1500, 1800, 2100, 2400], 1800]                         # 보유시간 제한(초)

# GA 최적화: 세대 수 × 개체 수 (예: 20세대 × 50개체 = 1,000회)
```
