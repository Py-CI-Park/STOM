...
본 문서는 STOM 주식 자동거래 시스템에서 **장 안정화 구간(09:30~10:00) 초기 모멘텀 지속 전략**을 정의한다.

- **대상 시간 구간**: 09:30:00 ~ 10:00:00 (장 안정화 구간)
- **대상 종목**: 관심종목, 가격대 1,000원~50,000원, 시가총액 차등 (2,500억/5,000억 기준)
- **전략 타입**: 초기 모멘텀 지속 + 추세 확인 (Early Momentum Continuation)
- **핵심 변수**: 시가등락율, 시가대비등락율, 초당순매수금액, 이동평균(30/60/120), 체결강도평균(60), 고가대비하락율, 거래대금안정성
- **업데이트 이력**:
  - 초기 문서 작성: 안정화 구간 모멘텀 지속 전략
...
# self.vars[4] 사용 - 값: 0.6
# self.vars[5] 사용 - 값: 0.7
# self.vars[6] 사용 - 값: 0.8
# self.vars[7] 사용 - 값: 1
# self.vars[8] 사용 - 값: 1.1
# self.vars[9] 사용 - 값: 1.2
```
...
self.vars[10] = [[1.0, 1.0, 1.0], 1.3]  # 카운트 (1.3)
self.vars[11] = [[1.0, 2.0, 1.0], 2.0]  # 카운트 (2.0)
self.vars[12] = [[2.0, 3.0, 1.0], 3.0]  # 카운트 (3.0)
self.vars[13] = [[2.0, 4.0, 1.0], 3.5]  # 카운트 (3.5)
self.vars[14] = [[2.0, 5.0, 1.0], 4.0]  # 카운트 (4.0)
self.vars[15] = [[3.0, 6.0, 1.0], 5.0]  # 카운트 (5.0)
self.vars[16] = [[10, 13, 5], 10]  # 임계값 (10)
self.vars[17] = [[14.0, 26.0, 5.0], 20.0]  # 임계값 (20.0)
self.vars[18] = [[21, 39, 5], 30]  # 임계값 (30)
self.vars[19] = [[28, 52, 5], 40]  # 임계값 (40)
```
