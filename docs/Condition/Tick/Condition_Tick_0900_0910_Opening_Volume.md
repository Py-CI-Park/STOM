...
if not (90000 <= 시분초 < 91000):
    ...
    매수 = False
elif not (self.vars[4] <= 시가등락율 <= 15.0):  # 시가등락율 하한 최적화
    매수 = False
elif not (self.vars[5] <= 시가대비등락율 <= 10.0):  # 시가대비등락율 하한 최적화
    매수 = False
elif not (데이터길이 > 5):
    매수 = False
elif not (초당거래대금 > 초당거래대금평균(10) * self.vars[6]):  # 초당거래대금 배수 최적화
    매수 = False
elif not (체결강도 > self.vars[7]):  # 체결강도 최적화
    매수 = False
elif not (초당매수수량 > 매도총잔량 * self.vars[8] / 100):  # 초당매수수량/매도총잔량 비율 최적화
    매수 = False
elif not (매수총잔량 > 매도총잔량 * 1.5):
    매수 = False
elif not (초당순매수금액 > self.vars[9]):  # 초당순매수금액 최적화
    매수 = False
elif 시가총액 < 1500:
    if not (전일동시간비 > 200):
        매수 = False
...
if 등락율 > 29.5:
    ...
    매도 = True
elif 보유시간 > self.vars[23]:  # 보유시간 제한 최적화
    매도 = True
elif 체결강도 < 80 and 수익률 > 1.0:
    매도 = True
elif (초당매도수량 - 초당매수수량) >= 매수총잔량 * 0.9 and 수익률 > 0.5:
...
```python
self.vars[0] = [[1000, 1500, 2000], 1500]         # 현재가 하한
self.vars[1] = [[15000, 20000, 25000], 20000]     # 현재가 상한
self.vars[2] = [[1.5, 2.0, 2.5], 2.0]             # 등락율 하한
self.vars[3] = [[27.0, 29.0, 29.5], 29.0]         # 등락율 상한
self.vars[4] = [[0.5, 1.0, 1.5], 1.0]             # 시가등락율 하한
self.vars[6] = [[3.0, 4.0, 5.0], 4.0]             # 초당거래대금 배수
self.vars[7] = [[180, 200, 220], 200]             # 체결강도
self.vars[9] = [[25, 30, 35], 30]                 # 초당순매수금액
self.vars[20] = [[3.0, 4.0, 5.0], 4.0]            # 목표 수익률
self.vars[21] = [[-4.0, -3.0, -2.0], -3.0]        # 손절 수익률
self.vars[22] = [[2.0, 2.5, 3.0], 2.5]            # 고점 대비 하락폭
```
