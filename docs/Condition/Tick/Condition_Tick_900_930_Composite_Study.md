...
본 문서는 STOM 주식 자동거래 시스템에서 **장 시작 30분(09:00~09:30) 종합 전략(Composite Study)**을 정의한다.

- **대상 시간 구간**: 09:00:00 ~ 09:30:00 (장 시작 30분 전체)
- **대상 종목**: 관심종목, 가격대 1,000원~50,000원, 시가총액 3,000억 미만
- **전략 타입**: 시간대별 복합 패턴 종합 (Composite Multi-Pattern)
- **핵심 변수**: 시가등락율, 시가대비등락율, 초당순매수금액, 초당거래대금, 체결강도, 이동평균, 아래꼬리비율, 고점돌파여부
- **업데이트 이력**:
  - 초기 문서 작성: 902/905/LongTail/Rebound/Trend 통합
...
# self.vars[2] 사용 - 값: 0.25
# self.vars[3] 사용 - 값: 0.3
# self.vars[4] 사용 - 값: 0.5
# self.vars[5] 사용 - 값: 0.8
# self.vars[6] 사용 - 값: 1
# self.vars[7] 사용 - 값: 1.1
# self.vars[8] 사용 - 값: 1.2
# self.vars[9] 사용 - 값: 1.5
```
...
self.vars[2] = [[0.2, 0.3, 0.1], 0.25]  # 비율 (0.25)
self.vars[3] = [[0.2, 0.4, 0.1], 0.3]  # 비율 (0.3)
self.vars[4] = [[0.3, 0.7, 0.1], 0.5]  # 비율 (0.5)
self.vars[5] = [[0.6, 1.0, 0.1], 0.8]  # 비율 (0.8)
self.vars[6] = [[1, 1, 1], 1]  # 카운트 (1)
self.vars[7] = [[1.0, 1.0, 1.0], 1.1]  # 카운트 (1.1)
self.vars[8] = [[1.0, 1.0, 1.0], 1.2]  # 카운트 (1.2)
self.vars[9] = [[1.0, 1.0, 1.0], 1.5]  # 카운트 (1.5)
self.vars[10] = [[1.0, 2.0, 1.0], 2.0]  # 카운트 (2.0)
self.vars[11] = [[1.0, 3.0, 1.0], 2.5]  # 카운트 (2.5)
self.vars[12] = [[2, 3, 1], 3]  # 카운트 (3)
self.vars[13] = [[3.0, 5.0, 1.0], 4.5]  # 카운트 (4.5)
self.vars[14] = [[3, 6, 1], 5]  # 카운트 (5)
self.vars[15] = [[4.0, 7.0, 1.0], 6.0]  # 카운트 (6.0)
self.vars[16] = [[6.0, 11.0, 1.0], 9.0]  # 카운트 (9.0)
self.vars[17] = [[10.0, 13.0, 5.0], 10.0]  # 임계값 (10.0)
self.vars[18] = [[12.0, 23.0, 5.0], 18.0]  # 임계값 (18.0)
...
self.vars[23] = [[1.0, 2.0, 1.0], 2.0]  # 카운트 (2.0)
self.vars[24] = [[3.0, 6.0, 1.0], 5.0]  # 카운트 (5.0)
self.vars[25] = [[4.0, 7.0, 1.0], 6.0]  # 카운트 (6.0)
self.vars[26] = [[20.0, 38.0, 5.0], 29.5]  # 임계값 (29.5)
self.vars[27] = [[21, 39, 5], 30]  # 임계값 (30)
self.vars[28] = [[42, 78, 5], 60]  # 임계값 (60)
self.vars[29] = [[70, 130, 100], 100]  # 금액 (100)
self.vars[30] = [[63699, 118300, 1000], 91000]  # 대금 (91000)
self.vars[31] = [[65099, 120900, 1000], 93000]  # 대금 (93000)
```
...
self.vars[5] = [[4.0, 7.0, 1.0], 6.0]  # 카운트 (6.0)
self.vars[6] = [[0.1, 0.1, 0.1], 0.1]  # 비율 (0.1)
self.vars[7] = [[0.1, 0.3, 0.1], 0.2]  # 비율 (0.2)
self.vars[8] = [[0.2, 0.3, 0.1], 0.25]  # 비율 (0.25)
self.vars[9] = [[0.2, 0.4, 0.1], 0.3]  # 비율 (0.3)
```
...
self.vars[8] = [[0.2, 0.3], 0.25]  # 비율 (0.25)
self.vars[9] = [[0.2, 0.4], 0.3]  # 비율 (0.3)
```
