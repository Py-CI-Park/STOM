...
class StomLiveClient:
    ...
    def UpdateStomLiveData(self, data):
        ...
        if data6 is not None:
            df8 = pd.DataFrame(columns=['백테스트', '백파인더', '최적화OH', '최적화OV', '최적화OVC', '최적화OHT', '최적화OVT', '최적화OVCT',
                                        '최적화OG', '최적화OGV', '최적화OGVC', '최적화OC', '최적화OCV', '최적화OCVC', '전진분석RH', '전진분석RV', '전진분석RVC'])
            for i, d in enumerate(data6):
                df8.loc[i] = [int(x) for x in d.split(';')]

            tbk   = df8['백테스트'].iloc[:-1].sum()
            tbf   = df8['백파인더'].iloc[:-1].sum()
            toh   = df8['최적화OH'].iloc[:-1].sum()
            tov   = df8['최적화OV'].iloc[:-1].sum()
            tovc  = df8['최적화OVC'].iloc[:-1].sum()
            toht  = df8['최적화OHT'].iloc[:-1].sum()
            tovt  = df8['최적화OVT'].iloc[:-1].sum()
            tovct = df8['최적화OVCT'].iloc[:-1].sum()
            ...
            trh   = df8['전진분석RH'].iloc[:-1].sum()
            trv   = df8['전진분석RV'].iloc[:-1].sum()
            trvc  = df8['전진분석RVC'].iloc[:-1].sum()
            ...
            aoh   = df8['최적화OH'].iloc[:-1].mean()
            aov   = df8['최적화OV'].iloc[:-1].mean()
            aovc  = df8['최적화OVC'].iloc[:-1].mean()
            aoht  = df8['최적화OHT'].iloc[:-1].mean()
            aovt  = df8['최적화OVT'].iloc[:-1].mean()
            aovct = df8['최적화OVCT'].iloc[:-1].mean()
            ...
            arh   = df8['전진분석RH'].iloc[:-1].mean()
            arv   = df8['전진분석RV'].iloc[:-1].mean()
            arvc  = df8['전진분석RVC'].iloc[:-1].mean()
