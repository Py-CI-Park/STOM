...
if 'back' not in table_list:
    columns = [
        ...
        "백테스케쥴시간", "백테스케쥴구분", "백테스케쥴명", "백테날짜고정", "백테날짜", "최적화기준값제한", "백테엔진분류방법", "옵튜나샘플러"
    ]
    data = [0, 0, 0, 0, 1, 0, 0, 5, 0, 0, 154000, 0, 0, 5, 0, 0, 0, 1, 1, 0, 4, 160000, '', '', 1, '20220323', '0.0;1000.0;0;100.0;0.0;100.0;-10.0;10.0;0.0;100.0;-10000.0;10000.0;0.0;10.0', '종목코드별 분류', 'BaseSampler']
    df = pd.DataFrame([data], columns=columns).set_index('index')
    df.to_sql('back', con)
else:
    df = pd.read_sql('SELECT * FROM back', con).set_index('index')
    if '옵튜나샘플러' not in df.columns:
