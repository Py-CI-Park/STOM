...
BB_중심돌파 = (현재가 > BB_중심) and (현재가_N(1) <= BB_중심_이전)  # 중심선 상향 돌파
BB_하단반등 = (현재가 > BB_하단 * 1.01) and (현재가_N(1) <= BB_하단_이전 * 1.01)
...
# 추가 기술적 지표 확인
elif not (RSI(14) >= 25 and RSI(14) <= 70):  # RSI 조건
    매수 = False
...
# 거래량 및 강도 조건
    ...
    매수 = False
elif not (분당매수량 > 분당매도량 * 0.8):  # 매수 우세 (여유있게)
    매수 = False
...
if not 매수:
    ...
    # 볼린저밴드 워킹 더 밴드 시작 패턴
        ...
        매수 = True
    # 볼린저밴드 반등 확인 패턴
    elif BB_하단터치 and (체결강도 > 110) and (현재가 > 현재가_N(2)):
        매수 = True
...
BB_중심하락돌파 = (현재가 < BB_중심) and (현재가_N(1) >= BB_중심_이전)  # 중심선 하향 돌파
...
# 볼린저밴드 기반 위험 신호
    ...
    매도 = True
elif (현재가 < BB_상단 * 0.98) and (현재가_N(1) >= BB_상단_이전 * 0.98):  # 상단 이탈
    매도 = True
