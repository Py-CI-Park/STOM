...
class CoinFutureBackEngine2(CoinFutureBackEngine):
            ...
            for j in range(self.vars_count):
                ...
                if self.dict_set['코인매수주문구분'] == '시장가':
                    if not self.trade_info[j]['보유중']:
                        gubun = 'BUY_LONG or SELL_SHORT'
                    elif self.trade_info[j]['매수분할횟수'] < self.dict_set['코인매수분할횟수']:
                        gubun = 'BUY_LONG or SELL_SHORT'
                    else:
                        gubun = 'SELL_LONG or BUY_SHORT'
                elif self.dict_set['코인매수주문구분'] == '지정가':
                    if not self.trade_info[j]['보유중']:
                        if self.trade_info[j]['롱매수호가'] == 0 and self.trade_info[j]['숏매수호가'] == 0:
                            gubun = 'BUY_LONG or SELL_SHORT'
                        else:
                            관심이탈 = not 관심종목 and 관심종목N(1)
                            self.CheckBuy('LONG' if self.trade_info[j]['롱매수호가'] != 0 else 'SHORT', 현재가, 관심이탈)
                            continue
                    elif self.trade_info[j]['매수분할횟수'] < self.dict_set['코인매수분할횟수']:
                        if self.trade_info[j]['롱매수호가'] == 0 and self.trade_info[j]['숏매수호가'] == 0:
                            gubun = 'BUY_LONG or SELL_SHORT'
                            ...
                            continue
                        if self.trade_info[j]['롱매수호가'] == 0 and self.trade_info[j]['숏매수호가'] == 0:
                            if self.trade_info[j]['롱매도호가'] == 0 and self.trade_info[j]['숏매도호가'] == 0:
                                gubun = 'SELL_LONG or BUY_SHORT'
                            else:
                                관심진입 = 관심종목 and not 관심종목N(1)
                                self.CheckSell('LONG' if self.trade_info[j]['롱매도호가'] != 0 else 'SHORT', 현재가, 관심진입)
                                continue
                    else:
                        if self.trade_info[j]['롱매도호가'] == 0 and self.trade_info[j]['숏매도호가'] == 0:
                            gubun = 'SELL_LONG or BUY_SHORT'
                ...
                try:
                    if gubun == 'BUY_LONG or SELL_SHORT':
                        ...
                        if self.dict_set['코인매수분할횟수'] == 1:
                            ...
                            self.trade_info[j]['주문수량'] = round(self.betting / (현재가 if 포지션 is None else self.trade_info[j]['매수가']) * oc_ratio / 100, 8)
    ...
    def Buy(self, gubun):
        if self.pattern:
            self.PatternModeling('매수' if gubun == 'BUY_LONG' else '매도')
        if self.pattern_test:
            pattern = self.GetPattern('매수' if gubun == 'BUY_LONG' else '매도')
    ...
    def CheckBuy(self, gubun, 현재가, 관심이탈):
        if self.dict_set['코인매수취소관심이탈'] and 관심이탈:
            ...
            self.trade_info[self.vars_key]['매수정정횟수'] += 1
        elif (gubun == 'LONG' and 현재가 < self.trade_info[self.vars_key]['롱매수호가']) or \
                (gubun == 'SHORT' and 현재가 > self.trade_info[self.vars_key]['숏매수호가']):
            직전매수가 = self.trade_info[self.vars_key]['매수가']
            보유수량 = self.trade_info[self.vars_key]['보유수량']
            직전매수금액 = 직전매수가 * 보유수량
            주문수량 = self.trade_info[self.vars_key]['주문수량']
            추가매수가 = self.trade_info[self.vars_key]['롱매수호가' if gubun == 'LONG' else '숏매수호가']
            매수금액 = 추가매수가 * 주문수량
            총수량 = 보유수량 + 주문수량
            평단가 = round((직전매수금액 + 매수금액) / 총수량, 4)
            self.trade_info[self.vars_key]['매수가'] = 평단가
    ...
    def Sell(self, gubun, sell_cond):
        if self.pattern:
            self.PatternModeling('매도' if gubun == 'SELL_LONG' else '매수')
        if self.pattern_test:
            pattern = self.GetPattern('매도' if gubun == 'SELL_LONG' else '매수')
    ...
    def CheckSell(self, gubun, 현재가, 관심진입):
