...
class CoinFutureBackEngine:
    ...
    def Buy(self, gubun):
        ...
        if 매수수량 > 0:
            ...
            if 남은수량 <= 0:
                ...
                self.trade_info['롱보유중' if gubun == 'BUY_LONG' else '숏보유중'] = True
    ...
    def CalculationEyun(self):
        ...
        self.tq.put(['백테결과', self.code, position, int(self.array_tick[self.indexb, 0]), self.index, hold_sec, self.trade_info['매수가'],
                     self.trade_info['매도가'], bg, pg, sp, sg, self.dict_cond[self.sell_cond], '', True])
