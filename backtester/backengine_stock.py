...
from utility.static import strp_time, timedelta_sec, pickle_write, pickle_read, GetKiwoomPgSgSp, GetUvilower5, GetHogaunit


class StockBackEngine:
    def __init__(self, gubun, wq, pq, tq, bq, profile=False):
        self.gubun       = gubun
        self.wq          = wq
        self.pq          = pq
        self.tq          = tq
    ...
    def CalculationEyun(self):
        ...
        self.tq.put(['백테결과', self.code, int(self.array_tick[self.indexn, 12]), int(self.array_tick[self.indexb, 0]),
                     ...
                     self.dict_cond[self.sell_cond], '', True])
