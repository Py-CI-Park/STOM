...
from .utils import round_series, DEFAULT_DECIMAL_PLACES
...
def CalculateEnhancedDerivedMetrics(df_tsg, timeframe: str = 'auto', save_file_name: str = ''):
    ...
    # =============================================================================
    # [2026-01-07] 소수점 4자리 제한 적용 (파생 변수 일괄 반올림)
    # =============================================================================
    # 비율, 퍼센트, 점수 등 연산으로 생성된 파생 변수의 소수점을 4자리로 제한합니다.
    # - 목적: CSV 출력 시 가독성 향상, 파일 크기 감소
    # - 대상: 나눗셈/곱셈으로 생성된 비율/퍼센트/점수 컬럼
    ...
    ratio_columns = [
        ...
        '거래대금변화율', '체결강도변화율',
        # 점수 지표
        '모멘텀점수', '위험도점수', '매수매도위험도점수', '거래품질점수',
        '타이밍점수', '시간대평균수익률', '리스크조정수익률',
        # 변동폭 비율
        '매수변동폭비율', '매도변동폭비율', '변동성변화',
        # 틱 모드 전용 비율
        '초당매수수량_매도총잔량_비율', '매도잔량_매수잔량_비율', '매수잔량_매도잔량_비율',
        '초당매도_매수_비율', '초당매수_매도_비율', '현재가_고저범위_위치',
        '초당거래대금_당일비중', '초당순매수금액', '초당순매수비율',
        '매도시_초당매수_매도_비율', '초당거래대금변화율',
        # 분봉 모드 전용 비율
        '분당매수수량_매도총잔량_비율', '분당매도_매수_비율', '분당매수_매도_비율',
        '현재가_분봉고저범위_위치', '분당거래대금_당일비중',
        '분당순매수금액', '분당순매수비율',
        '매도시_분당매수_매도_비율', '분당거래대금변화율',
        # 당일거래대금 비율
        '당일거래대금_전틱분봉_비율', '당일거래대금_5틱분봉평균_비율', '당일거래대금_매수매도_비율',
    ]
    
    for col in ratio_columns:
        if col in df.columns:
            df[col] = round_series(df[col], DEFAULT_DECIMAL_PLACES)
