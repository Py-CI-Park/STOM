...
class StockBackEngine2(StockBackEngine):
        ...
        if 주문수량 > 0:
            if self.dict_set['주식매수주문구분'] == '시장가':
                ...
                self.trade_info[vars_turn][vars_key]['매수호가단위'] = (
                    GetHogaunit(self.dict_kd[self.code] if self.code in self.dict_kd.keys() else True, self.array_tick[self.indexn, 1], self.index))
    ...
    def CheckBuy(self, vars_turn, vars_key, 현재가, 관심이탈):
        ...
        if self.dict_set['주식매수취소관심이탈'] and 관심이탈:
            ...
            매수호가단위 = GetHogaunit(self.dict_kd[self.code] if self.code in self.dict_kd.keys() else True, 현재가, self.index)
