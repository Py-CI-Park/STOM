...
from utility.setting import DB_STOCK_BACK, BACK_TEMP, DICT_SET, ui_num, dict_order_ratio
from utility.static import strp_time, timedelta_sec, roundfigure_upper, roundfigure_lower, pickle_write, pickle_read, GetKiwoomPgSgSp, GetUvilower5, GetHogaunit


class StockBackEngine2:
    ...
    def BackTest(self):
        ...
        for code in self.code_list:
            ...
            if self.dict_set['주식매수금지블랙리스트'] and self.code in self.dict_set['주식블랙리스트'] and self.back_type != '백파인더':
                self.tq.put(['백테완료', 0])
                continue
    ...
    def Strategy(self, gubun):
        ...
        if self.back_type == '백파인더':
            ...
            if self.dict_set['주식매수분할횟수'] == 1:
                ...
                oc_ratio = dict_order_ratio[self.dict_set['주식매수분할방법']][self.dict_set['주식매수분할횟수']][self.trade_info['매수분할횟수']]
                self.trade_info['주문수량'] = int(self.betting / (현재가 if not self.trade_info['보유중'] else self.trade_info['매수가']) * oc_ratio / 100)
            ...
            if not self.trade_info['보유중']:
                ...
                if self.dict_set['주식매수분할하방'] and 분할매수기준수익률 < -self.dict_set['주식매수분할하방수익률']:  self.Buy()
                elif self.dict_set['주식매수분할상방'] and 분할매수기준수익률 > self.dict_set['주식매수분할상방수익률']: self.Buy()
            ...
            if self.dict_set['주식매도분할횟수'] == 1:
                ...
                if self.dict_set['주식매도분할하방'] and 수익률 < -self.dict_set['주식매도분할하방수익률'] * (self.trade_info['매도분할횟수'] + 1):  self.Sell(100)
                elif self.dict_set['주식매도분할상방'] and 수익률 > self.dict_set['주식매도분할상방수익률'] * (self.trade_info['매도분할횟수'] + 1): self.Sell(100)
    ...
    def Buy(self):
        if self.dict_set['주식매수주문구분'] == '시장가':
            ...
            self.trade_info['매수호가단위'] = GetHogaunit(self.dict_kd[self.code] if self.code in self.dict_kd.keys() else True, self.array_tick[self.indexn, 1], self.index)
