...
class BackSubTotal:
    ...
    def CollectData(self, data):
        ...
        # [2025-12-14] 초당(틱) 데이터 컬럼 추가 - 매수/매도 시점 초당매수/초당매도/초당거래대금
        당일거래대금_전틱분봉_비율 = 0.0
        당일거래대금_5틱분봉평균_비율 = 0.0
        if len(data) >= 61:  # 확장된 데이터 (17 + 22 + 3 + 16 + 3) - 매수 + 매도 + 초당 데이터
            data = data[:61]  # 이후 컬럼이 추가되더라도 언패킹 에러 방지
            _, 종목명, 시가총액또는포지션, 매수시간, 매도시간, 보유시간, 매수가, 매도가, 매수금액, 매도금액, 수익률, 수익금, 매도조건, \
                추가매수시간, 잔량없음, vturn, vkey, \
                매수일자, 매수시, 매수분, 매수초, \
                매수등락율, 매수시가등락율, 매수당일거래대금, 당일거래대금_전틱분봉_비율, 당일거래대금_5틱분봉평균_비율, 매수체결강도, \
                ...
                매도초당매수수량, 매도초당매도수량, 매도초당거래대금= data
        elif len(data) >= 59:  # 확장된 데이터 (17 + 20 + 3 + 16 + 3) - 매수 + 매도 + 초당 데이터
            data = data[:59]  # 이후 컬럼이 추가되더라도 언패킹 에러 방지
            _, 종목명, 시가총액또는포지션, 매수시간, 매도시간, 보유시간, 매수가, 매도가, 매수금액, 매도금액, 수익률, 수익금, 매도조건, \
                추가매수시간, 잔량없음, vturn, vkey, \
                매수일자, 매수시, 매수분, 매수초, \
                ...
                매도매도호가1, 매도매수호가1, 매도스프레드, \
                매도초당매수수량, 매도초당매도수량, 매도초당거래대금 = data
        elif len(data) >= 56:  # 확장된 데이터 (17 + 20 + 3 + 16) - 매수 + 매도 + (매수)초당 데이터만
            # 일부 백테/데이터 소스에서 매도 시점 초당 데이터가 누락될 수 있어 하위 호환 처리
            data = data[:56]  # 이후 컬럼이 추가되더라도 언패킹 에러 방지
            _, 종목명, 시가총액또는포지션, 매수시간, 매도시간, 보유시간, 매수가, 매도가, 매수금액, 매도금액, 수익률, 수익금, 매도조건, \
                추가매수시간, 잔량없음, vturn, vkey, \
                매수일자, 매수시, 매수분, 매수초, \
                매수등락율, 매수시가등락율, 매수당일거래대금, 매수체결강도, \
                ...
                매도고가, 매도저가, 매도고저평균대비등락율, \
                매도매도총잔량, 매도매수총잔량, 매도호가잔량비, \
                매도매도호가1, 매도매수호가1, 매도스프레드 = data
            매도초당매수수량, 매도초당매도수량, 매도초당거래대금 = 0, 0, 0.0
        elif len(data) >= 53:  # 확장된 데이터 (17 + 20 + 16) - 매수 + 매도 시점 데이터
            data = data[:53]  # 이후 컬럼이 추가되더라도 언패킹 에러 방지
            _, 종목명, 시가총액또는포지션, 매수시간, 매도시간, 보유시간, 매수가, 매도가, 매수금액, 매도금액, 수익률, 수익금, 매도조건, \
                추가매수시간, 잔량없음, vturn, vkey, \
                매수일자, 매수시, 매수분, 매수초, \
                매수등락율, 매수시가등락율, 매수당일거래대금, 매수체결강도, \
                ...
                매도매도총잔량, 매도매수총잔량, 매도호가잔량비, \
                매도매도호가1, 매도매수호가1, 매도스프레드 = data
            매수초당매수수량, 매수초당매도수량, 매수초당거래대금 = 0, 0, 0.0
            매도초당매수수량, 매도초당매도수량, 매도초당거래대금 = 0, 0, 0.0
        elif len(data) >= 37:  # 매수 시점 데이터만 (하위 호환성)
            data = data[:37]  # 이후 컬럼이 추가되더라도 언패킹 에러 방지
            _, 종목명, 시가총액또는포지션, 매수시간, 매도시간, 보유시간, 매수가, 매도가, 매수금액, 매도금액, 수익률, 수익금, 매도조건, \
                추가매수시간, 잔량없음, vturn, vkey, \
                매수일자, 매수시, 매수분, 매수초, \
                매수등락율, 매수시가등락율, 매수당일거래대금, 매수체결강도, \
            ...
            매수초당매수수량, 매수초당매도수량, 매수초당거래대금 = 0, 0, 0.0
            매도등락율, 매도시가등락율, 매도당일거래대금, 매도체결강도 = 0.0, 0.0, 0, 0.0
            매도전일비, 매도회전율, 매도전일동시간비 = 0.0, 0.0, 0.0
            매도고가, 매도저가, 매도고저평균대비등락율 = 0, 0, 0.0
            매도매도총잔량, 매도매수총잔량, 매도호가잔량비 = 0, 0, 0.0
            매도매도호가1, 매도매수호가1, 매도스프레드 = 0, 0, 0.0
            매도초당매수수량, 매도초당매도수량, 매도초당거래대금 = 0, 0, 0.0
        else:  # 기존 데이터 (하위 호환성)
            data = data[:17]  # 이후 컬럼이 추가되더라도 언패킹 에러 방지
            _, 종목명, 시가총액또는포지션, 매수시간, 매도시간, 보유시간, 매수가, 매도가, 매수금액, 매도금액, 수익률, 수익금, 매도조건, \
