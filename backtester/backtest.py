...
from backtester.back_static import GetBackResult, PltShow, GetQuery
from ui.set_text import example_stock_buy, example_stock_sell
from utility.static import strf_time, strp_time, now, timedelta_sec
from utility.setting import DB_STRATEGY, DB_BACKTEST, ui_num, stockreadlines, columns_vj, DICT_SET, DB_STOCK_BACK, \
    DB_COIN_BACK, coinreadlines


class Total:
    def __init__(self, wq, sq, tq, mq, lq, bq, backname, ui_gubun, gubun):
        self.wq           = wq
        self.sq           = sq
        self.tq           = tq
        self.mq           = mq
...
class BackTest:
    ...
    def Start(self):
        ...
        self.wq.put([ui_num[f'{self.ui_gubun}백테스트'], f'{self.backname} 매도수전략 로딩 완료'])

        mq = Queue()
        Process(target=Total, args=(self.wq, self.sq, self.tq, mq, self.lq, self.bq, self.backname, self.ui_gubun, self.gubun)).start()
        self.wq.put([ui_num[f'{self.ui_gubun}백테스트'], f'{self.backname} 집계용 프로세스 생성 완료'])

        self.tq.put(['백테정보', betting, avgtime, startday, endday, starttime, endtime, buystg_name, buystg, sellstg, dict_cn, back_count, day_count, bl, schedul, arry_bct, df_kp, df_kq, back_club])
        for pq in self.pq_list:
            pq.put(['백테정보', betting, avgtime, startday, endday, starttime, endtime, buystg, sellstg])
